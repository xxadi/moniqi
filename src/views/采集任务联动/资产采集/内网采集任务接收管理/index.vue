    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("定义内网采集任务接收信息", "operation")'>定义内网采集任务接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集任务接收信息", "edit")'>修改内网采集任务接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集任务接收信息", "delete")'>删除内网采集任务接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("内网采集任务接收数据获取", "operation")'>内网采集任务接收数据获取</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("内网采集任务接收数据锁定", "operation")'>内网采集任务接收数据锁定</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("内网采集任务接收数据激活", "operation")'>内网采集任务接收数据激活</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询内网采集任务接收报告", "detail")'>查询内网采集任务接收报告</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集任务接收状态查询信息", "add")'>新增内网采集任务接收状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集任务接收状态查询信息", "edit")'>修改内网采集任务接收状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集任务接收状态查询信息", "delete")'>删除内网采集任务接收状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集任务接收参数信息", "add")'>新增内网采集任务接收参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集任务接收参数信息", "edit")'>修改内网采集任务接收参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集任务接收参数信息", "delete")'>删除内网采集任务接收参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集任务接收错误处理信息", "add")'>新增内网采集任务接收错误处理信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集任务接收错误事件通知规则", "add")'>新增内网采集任务接收错误事件通知规则</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "内网采集任务接收管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "内网采集任务接收管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "内网采集任务接收管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "内网采集任务接收管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

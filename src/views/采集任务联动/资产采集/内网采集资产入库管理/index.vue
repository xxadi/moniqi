    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("定义内网采集资产入库信息", "operation")'>定义内网采集资产入库信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集资产入库信息", "edit")'>修改内网采集资产入库信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集资产入库信息", "delete")'>删除内网采集资产入库信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("内网采集资产入库数据获取", "operation")'>内网采集资产入库数据获取</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("内网采集资产入库数据锁定", "operation")'>内网采集资产入库数据锁定</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("内网采集资产入库数据激活", "operation")'>内网采集资产入库数据激活</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询内网采集资产入库报告", "detail")'>查询内网采集资产入库报告</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集资产入库状态查询信息", "add")'>新增内网采集资产入库状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集资产入库状态查询信息", "edit")'>修改内网采集资产入库状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集资产入库状态查询信息", "delete")'>删除内网采集资产入库状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集资产入库参数信息", "add")'>新增内网采集资产入库参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集资产入库参数信息", "edit")'>修改内网采集资产入库参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集资产入库参数信息", "delete")'>删除内网采集资产入库参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集资产入库错误处理信息", "add")'>新增内网采集资产入库错误处理信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集资产入库错误事件通知规则", "add")'>新增内网采集资产入库错误事件通知规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "入库耗时": "3.5s",
  "入库状态": "入库中",
  "完成时间": "2026-05-08 14:03:52",
  "ID": "",
  "导入来源": "内网采集资产入库管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage190",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "内网采集资产入库管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "内网采集资产入库管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "内网采集资产入库管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "内网采集资产入库管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "内网采集资产入库管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "内网采集资产入库管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "内网采集资产入库管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "内网采集资产入库管理" },
        { label: "一级模块", value: "采集任务联动" },
        { label: "二级模块", value: "资产采集" },
      ];

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("启动对采集指令内容的字段解析", "add")'>启动对采集指令内容的字段解析</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载采集器调用参数配置", "export")'>下载采集器调用参数配置</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存解析结果与参数结构", "operation")'>保存解析结果与参数结构</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询采集器支持的参数规则", "detail")'>查询采集器支持的参数规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("添加字段映射关系及约束配置", "edit")'>添加字段映射关系及约束配置</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "入库状态": "已回退",
  "ID": "",
  "导入来源": "解析采集指令参数导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage181",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "解析采集指令参数导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "解析采集指令参数任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "解析采集指令参数异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "解析采集指令参数状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "解析采集指令参数回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "解析采集指令参数业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "解析采集指令参数复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

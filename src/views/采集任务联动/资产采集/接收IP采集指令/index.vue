    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("接收工信部或调度平台下发的IP采集任务", "operation")'>接收工信部或调度平台下发的IP采集任务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("响应接收结果并记录下发日志", "operation")'>响应接收结果并记录下发日志</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存指令原始信息及状态", "operation")'>保存指令原始信息及状态</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询本地是否已有重复指令", "detail")'>查询本地是否已有重复指令</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加指令格式与接口规范模板", "add")'>添加指令格式与接口规范模板</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "入库状态": "部分入库",
  "ID": "",
  "导入来源": "接收IP采集指令导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage180",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "接收IP采集指令导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "接收IP采集指令任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "接收IP采集指令异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "接收IP采集指令状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "接收IP采集指令回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "接收IP采集指令业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "接收IP采集指令复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

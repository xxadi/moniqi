    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("创建采集日志记录任务", "add")'>创建采集日志记录任务</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载采集日志写入状态", "export")'>下载采集日志写入状态</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存日志内容与采集时间戳", "operation")'>保存日志内容与采集时间戳</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询任务日志归档情况", "detail")'>查询任务日志归档情况</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加采集日志格式标准", "add")'>添加采集日志格式标准</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 10:08:06",
  "ID": "",
  "导入来源": "记录采集任务日志导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage187",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "记录采集任务日志导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "记录采集任务日志任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "记录采集任务日志异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "记录采集任务日志状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "记录采集任务日志回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "记录采集任务日志业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "记录采集任务日志复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

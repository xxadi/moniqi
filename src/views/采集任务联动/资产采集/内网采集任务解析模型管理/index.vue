    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集任务解析模型", "add")'>新增内网采集任务解析模型</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集任务解析模型", "edit")'>修改内网采集任务解析模型</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集任务解析模型", "delete")'>删除内网采集任务解析模型</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询内网采集任务解析模型", "detail")'>查询内网采集任务解析模型</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("内网采集任务解析模型启用/禁用", "operation")'>内网采集任务解析模型启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("内网采集任务解析模型运行效果评估", "analysis")'>内网采集任务解析模型运行效果评估</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("内网采集任务解析模型分析", "analysis")'>内网采集任务解析模型分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("内网采集任务解析模型分析结果展示", "detail")'>内网采集任务解析模型分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("内网采集任务解析模型分析结果下载", "export")'>内网采集任务解析模型分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("内网采集任务解析模型分析结果上报", "analysis")'>内网采集任务解析模型分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "入库状态": "待审批入库",
  "ID": "",
  "导入来源": "内网采集任务解析模型管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage184",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "内网采集任务解析模型管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "内网采集任务解析模型管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "内网采集任务解析模型管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "内网采集任务解析模型管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "内网采集任务解析模型管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "内网采集任务解析模型管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "内网采集任务解析模型管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

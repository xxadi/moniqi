    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("工信部内网采集联动接口", "operation")'>工信部内网采集联动接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("工信部内网采集联动数据分析", "analysis")'>工信部内网采集联动数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看工信部内网采集联动信息", "detail")'>查看工信部内网采集联动信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("工信部内网采集联动数据导出", "export")'>工信部内网采集联动数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增工信部内网采集联动状态查询信息", "add")'>新增工信部内网采集联动状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改工信部内网采集联动状态查询信息", "edit")'>修改工信部内网采集联动状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除工信部内网采集联动状态查询信息", "delete")'>删除工信部内网采集联动状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增工信部内网采集联动参数信息", "add")'>新增工信部内网采集联动参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改工信部内网采集联动参数信息", "edit")'>修改工信部内网采集联动参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除工信部内网采集联动参数信息", "delete")'>删除工信部内网采集联动参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("工信部内网采集联动接口调用查询", "detail")'>工信部内网采集联动接口调用查询</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("工信部内网采集联动调用统计", "analysis")'>工信部内网采集联动调用统计</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "工信部内网采集联动接口状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "工信部内网采集联动接口回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "工信部内网采集联动接口业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "工信部内网采集联动接口复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("采集结果可视化展示", "detail")'>采集结果可视化展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载图形化报告和采集结果", "export")'>下载图形化报告和采集结果</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("保存图表样式与展示配置", "edit")'>保存图表样式与展示配置</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询历史采集任务的可视化记录", "detail")'>查询历史采集任务的可视化记录</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加可视化组件与报告模板", "add")'>添加可视化组件与报告模板</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 16:08:25",
  "ID": "",
  "导入来源": "结果可视化展示导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage192",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "结果可视化展示导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "结果可视化展示任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "结果可视化展示异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "结果可视化展示状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "结果可视化展示回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "结果可视化展示业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "结果可视化展示复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

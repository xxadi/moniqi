    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("启动任务进度跟踪", "add")'>启动任务进度跟踪</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载进度更新信息与执行情况", "export")'>下载进度更新信息与执行情况</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("保存进度更新记录与任务状态", "edit")'>保存进度更新记录与任务状态</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询指定任务的进度数据", "detail")'>查询指定任务的进度数据</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("添加进度监控配置与展示模板", "edit")'>添加进度监控配置与展示模板</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 16:08:25",
  "ID": "",
  "导入来源": "采集任务进度跟踪导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage199",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "采集任务进度跟踪导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "采集任务进度跟踪任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "采集任务进度跟踪异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "采集任务进度跟踪状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "采集任务进度跟踪回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "采集任务进度跟踪业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "采集任务进度跟踪复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

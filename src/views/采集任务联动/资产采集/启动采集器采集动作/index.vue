    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("执行采集器调度指令", "operation")'>执行采集器调度指令</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加采集执行日志或状态码", "add")'>添加采集执行日志或状态码</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存采集进度与采集目标IP", "operation")'>保存采集进度与采集目标IP</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询采集任务实时进度", "detail")'>查询采集任务实时进度</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加采集器运行参数模板", "add")'>添加采集器运行参数模板</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "启动采集器采集动作导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage186",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "启动采集器采集动作导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "启动采集器采集动作任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "启动采集器采集动作异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "启动采集器采集动作状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "启动采集器采集动作回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "启动采集器采集动作业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "启动采集器采集动作复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

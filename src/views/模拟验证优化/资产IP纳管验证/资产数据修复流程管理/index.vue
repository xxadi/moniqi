    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("发起资产数据修复流程", "add")'>发起资产数据修复流程</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增资产数据修复指派信息", "add")'>新增资产数据修复指派信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增资产数据修复流程节点", "add")'>新增资产数据修复流程节点</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改资产数据修复流程节点信息", "edit")'>修改资产数据修复流程节点信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("资产数据修复流程节点删除", "delete")'>资产数据修复流程节点删除</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产数据修复流程节点查询", "detail")'>资产数据修复流程节点查询</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产数据修复流程图查看", "detail")'>资产数据修复流程图查看</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产数据修复上报接口", "operation")'>资产数据修复上报接口</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产数据修复结果逐级审批", "operation")'>资产数据修复结果逐级审批</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产数据修复结果拒绝和驳回", "operation")'>资产数据修复结果拒绝和驳回</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 09:35:44",
  "ID": "",
  "导入来源": "资产数据修复流程管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage256",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产数据修复流程管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产数据修复流程管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产数据修复流程管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "资产数据修复流程管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "资产数据修复流程管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产数据修复流程管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "资产数据修复流程管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增校验异常数据监控信息", "add")'>新增校验异常数据监控信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改校验异常数据监控信息", "edit")'>修改校验异常数据监控信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("校验失败数据删除", "delete")'>校验失败数据删除</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询校验异常数据监控列表", "detail")'>查询校验异常数据监控列表</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("校验异常数据反馈接口", "operation")'>校验异常数据反馈接口</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("批量导入校验异常数据监控", "import")'>批量导入校验异常数据监控</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("校验异常数据监控导出", "export")'>校验异常数据监控导出</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "检测时间": "2026-05-08 10:08:06",
  "ID": "",
  "导入来源": "校验异常数据监控管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage246",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "校验异常数据监控管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "校验异常数据监控管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "校验异常数据监控管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "校验异常数据监控管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "校验异常数据监控管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "校验异常数据监控管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "校验异常数据监控管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

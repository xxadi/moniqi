    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("资产指纹哈希生成接口管理", "add")'>资产指纹哈希生成接口管理</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增资产指纹哈希生成接口信息", "add")'>新增资产指纹哈希生成接口信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改资产指纹哈希生成接口信息", "edit")'>修改资产指纹哈希生成接口信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除资产指纹哈希生成接口信息", "delete")'>删除资产指纹哈希生成接口信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("资产指纹哈希生成接口信息查询", "add")'>资产指纹哈希生成接口信息查询</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("资产指纹哈希生成接口信息统计", "add")'>资产指纹哈希生成接口信息统计</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("资产指纹哈希生成接口数据导出", "export")'>资产指纹哈希生成接口数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("资产指纹哈希生成接口审核", "add")'>资产指纹哈希生成接口审核</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("资产指纹哈希生成接口明细检索", "add")'>资产指纹哈希生成接口明细检索</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("资产指纹哈希生成接口情况统计", "add")'>资产指纹哈希生成接口情况统计</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "最近调用": "2026-05-08 09:35:44",
  "ID": "",
  "导入来源": "资产指纹哈希生成接口管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage273",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产指纹哈希生成接口管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产指纹哈希生成接口管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产指纹哈希生成接口管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "资产指纹哈希生成接口管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "资产指纹哈希生成接口管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产指纹哈希生成接口管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "资产指纹哈希生成接口管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增校验异常反馈接口", "add")'>新增校验异常反馈接口</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("定义校验异常反馈信息", "operation")'>定义校验异常反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("校验异常反馈数据获取", "operation")'>校验异常反馈数据获取</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增校验异常反馈状态查询信息", "add")'>新增校验异常反馈状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("校验异常反馈结果监控", "detail")'>校验异常反馈结果监控</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "失败原因": "数据格式错误",
  "ID": "",
  "导入来源": "校验异常反馈接口管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage247",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "校验异常反馈接口管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "校验异常反馈接口管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "校验异常反馈接口管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "校验异常反馈接口管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "校验异常反馈接口管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "校验异常反馈接口管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "校验异常反馈接口管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

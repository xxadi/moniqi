    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指纹快速检索算法", "detail")'>指纹快速检索算法</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增指纹快速检索算法", "add")'>新增指纹快速检索算法</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改指纹快速检索算法", "edit")'>修改指纹快速检索算法</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除指纹快速检索算法", "delete")'>删除指纹快速检索算法</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指纹快速检索算法调用结果查询", "detail")'>指纹快速检索算法调用结果查询</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指纹快速检索算法调用结果统计", "detail")'>指纹快速检索算法调用结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("指纹快速检索算法训练数据导出", "export")'>指纹快速检索算法训练数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指纹快速检索算法应用情况", "detail")'>指纹快速检索算法应用情况</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指纹快速检索算法应用情况检索", "detail")'>指纹快速检索算法应用情况检索</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指纹快速检索算法应用情况统计", "detail")'>指纹快速检索算法应用情况统计</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "指纹快速检索算法导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage279",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指纹快速检索算法导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指纹快速检索算法任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指纹快速检索算法异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "指纹快速检索算法状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "指纹快速检索算法回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指纹快速检索算法业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "指纹快速检索算法复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

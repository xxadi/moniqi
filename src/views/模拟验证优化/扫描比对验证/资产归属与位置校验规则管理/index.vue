    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增资产归属与位置校验规则", "add")'>新增资产归属与位置校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改资产归属与位置校验规则", "edit")'>修改资产归属与位置校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除资产归属与位置校验规则", "delete")'>删除资产归属与位置校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询资产归属与位置校验规则", "detail")'>查询资产归属与位置校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产归属与位置校验规则启用/禁用", "operation")'>资产归属与位置校验规则启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产归属与位置校验失败告警", "operation")'>资产归属与位置校验失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归属与位置校验结果统计", "analysis")'>资产归属与位置校验结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产归属与位置校验数据提取", "operation")'>资产归属与位置校验数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归属与位置校验规则分析", "analysis")'>资产归属与位置校验规则分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产归属与位置校验规则分析结果展示", "detail")'>资产归属与位置校验规则分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("资产归属与位置校验规则分析结果下载", "export")'>资产归属与位置校验规则分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归属与位置校验规则分析结果上报", "analysis")'>资产归属与位置校验规则分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "资产归属与位置校验规则管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "资产归属与位置校验规则管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产归属与位置校验规则管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "资产归属与位置校验规则管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("接收工信部的日志接收反馈信息", "operation")'>接收工信部的日志接收反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载工信部的日志反馈状态", "export")'>下载工信部的日志反馈状态</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存接收到的反馈信息及处理记录", "operation")'>保存接收到的反馈信息及处理记录</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询日志反馈历史记录与状态", "detail")'>查询日志反馈历史记录与状态</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加日志反馈格式与处理接口", "add")'>添加日志反馈格式与处理接口</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "入库状态": "部分入库",
  "ID": "",
  "导入来源": "任务日志反馈接收导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage220",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "任务日志反馈接收导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "任务日志反馈接收任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "任务日志反馈接收异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "任务日志反馈接收状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "任务日志反馈接收回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "任务日志反馈接收业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "任务日志反馈接收复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

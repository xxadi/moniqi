    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("将采集任务的日志数据写入taskLog.log", "operation")'>将采集任务的日志数据写入taskLog.log</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加日志写入的状态及写入内容", "add")'>添加日志写入的状态及写入内容</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存日志写入操作及写入时间", "operation")'>保存日志写入操作及写入时间</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询日志写入历史记录与任务内容", "detail")'>查询日志写入历史记录与任务内容</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加日志写入格式及日志字段规范", "add")'>添加日志写入格式及日志字段规范</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "处理状态": "已升级",
  "ID": "",
  "导入来源": "写入采集日志导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage212",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "写入采集日志导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "写入采集日志任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "写入采集日志异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "写入采集日志状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "写入采集日志回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "写入采集日志业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "写入采集日志复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

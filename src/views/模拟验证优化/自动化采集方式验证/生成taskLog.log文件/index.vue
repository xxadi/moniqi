    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("创建taskLog.log文件", "add")'>创建taskLog.log文件</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加生成taskLog.log文件的状态和路径", "add")'>添加生成taskLog.log文件的状态和路径</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("保存taskLog.log文件生成记录及内容", "add")'>保存taskLog.log文件生成记录及内容</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询taskLog.log文件的生成历史记录", "add")'>查询taskLog.log文件的生成历史记录</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加taskLog文件的生成规则和格式模板", "add")'>添加taskLog文件的生成规则和格式模板</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "生成taskLog.log文件导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage211",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "生成taskLog.log文件导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "生成taskLog.log文件任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "生成taskLog.log文件异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "生成taskLog.log文件状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "生成taskLog.log文件回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "生成taskLog.log文件业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "生成taskLog.log文件复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "生成taskLog.log文件" },

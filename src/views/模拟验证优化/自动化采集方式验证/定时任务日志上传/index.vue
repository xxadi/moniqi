    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("定时上传任务日志到平台", "import")'>定时上传任务日志到平台</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("下载定时任务日志上传状态", "import")'>下载定时任务日志上传状态</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("保存定时上传任务日志记录", "import")'>保存定时上传任务日志记录</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("查询定时上传的任务日志记录", "import")'>查询定时上传的任务日志记录</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("添加定时上传任务日志规则", "import")'>添加定时上传任务日志规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 15:21:37",
  "ID": "",
  "导入来源": "定时任务日志上传导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage227",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "定时任务日志上传导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "定时任务日志上传任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "定时任务日志上传异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "定时任务日志上传状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "定时任务日志上传回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "定时任务日志上传业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "定时任务日志上传复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

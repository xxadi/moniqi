    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报内网IP采集任务的日志数据", "operation")'>上报内网IP采集任务的日志数据</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加日志上报结果及状态", "add")'>添加日志上报结果及状态</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存上报日志记录与状态反馈", "operation")'>保存上报日志记录与状态反馈</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询日志上报的历史记录及结果", "detail")'>查询日志上报的历史记录及结果</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加上报接口与工信部的标准", "add")'>添加上报接口与工信部的标准</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "上报时间": "2026-05-08 09:35:44",
  "ID": "",
  "导入来源": "内网IP采集日志上报导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage216",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "内网IP采集日志上报导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "内网IP采集日志上报任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "内网IP采集日志上报异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "内网IP采集日志上报状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "内网IP采集日志上报回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "内网IP采集日志上报业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "内网IP采集日志上报复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

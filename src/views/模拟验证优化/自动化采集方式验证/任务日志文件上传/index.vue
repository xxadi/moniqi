    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("上传任务日志文件到平台", "import")'>上传任务日志文件到平台</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("下载任务日志上传状态与结果", "import")'>下载任务日志上传状态与结果</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("保存上传日志记录与上传时间", "import")'>保存上传日志记录与上传时间</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("查询日志上传历史与文件状态", "import")'>查询日志上传历史与文件状态</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("添加上传文件规则与接口", "import")'>添加上传文件规则与接口</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "任务日志文件上传导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage223",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "任务日志文件上传导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "任务日志文件上传任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "任务日志文件上传异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "任务日志文件上传状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "任务日志文件上传回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "任务日志文件上传业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "任务日志文件上传复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

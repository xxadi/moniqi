    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("确认任务日志文件的下载请求", "export")'>确认任务日志文件的下载请求</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载下载确认信息与状态", "export")'>下载下载确认信息与状态</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("保存下载请求记录与确认状态", "export")'>保存下载请求记录与确认状态</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("查询下载确认历史记录", "export")'>查询下载确认历史记录</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("添加下载确认处理规则", "export")'>添加下载确认处理规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "任务日志文件下载确认导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage224",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "任务日志文件下载确认导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "任务日志文件下载确认任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "任务日志文件下载确认异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "任务日志文件下载确认状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "任务日志文件下载确认回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "任务日志文件下载确认业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "任务日志文件下载确认复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

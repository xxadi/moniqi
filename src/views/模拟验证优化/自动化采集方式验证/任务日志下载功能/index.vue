    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("提供下载采集任务日志的接口", "export")'>提供下载采集任务日志的接口</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("添加下载任务日志的状态与文件路径", "export")'>添加下载任务日志的状态与文件路径</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("保存下载请求及下载文件记录", "export")'>保存下载请求及下载文件记录</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("查询历史日志下载记录与文件状态", "export")'>查询历史日志下载记录与文件状态</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("添加下载文件格式与规范", "export")'>添加下载文件格式与规范</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "任务日志下载功能导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage217",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "任务日志下载功能导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "任务日志下载功能任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "任务日志下载功能异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "任务日志下载功能状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "任务日志下载功能回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "任务日志下载功能业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "任务日志下载功能复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增资产自定义项校验信息", "add")'>新增资产自定义项校验信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改资产自定义项校验信息", "edit")'>修改资产自定义项校验信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除资产自定义项校验信息", "delete")'>删除资产自定义项校验信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看资产自定义项校验信息", "detail")'>查看资产自定义项校验信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项校验数据提取", "operation")'>资产自定义项校验数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项校验数据识别", "operation")'>资产自定义项校验数据识别</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项校验清洗", "operation")'>资产自定义项校验清洗</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项校验分级", "operation")'>资产自定义项校验分级</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项校验实时计算", "operation")'>资产自定义项校验实时计算</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项校验聚合", "operation")'>资产自定义项校验聚合</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("资产自定义项校验信息导出", "export")'>资产自定义项校验信息导出</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产自定义项校验可视化展示", "detail")'>资产自定义项校验可视化展示</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项校验数据审查", "operation")'>资产自定义项校验数据审查</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "所属区域": "DMZ区域",
  "纳管状态": "待核验",
  "最近同步": "2026-05-08 10:42:31",
  "ID": "",
  "导入来源": "资产自定义项校验报表导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage292",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产自定义项校验报表导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产自定义项校验报表任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产自定义项校验报表异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "资产自定义项校验报表状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "资产自定义项校验报表回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产自定义项校验报表业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "资产自定义项校验报表复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

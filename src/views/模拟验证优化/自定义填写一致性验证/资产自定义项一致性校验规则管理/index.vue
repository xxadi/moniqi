    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增资产自定义项一致性校验规则", "add")'>新增资产自定义项一致性校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改资产自定义项一致性校验规则", "edit")'>修改资产自定义项一致性校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除资产自定义项一致性校验规则", "delete")'>删除资产自定义项一致性校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询资产自定义项一致性校验规则", "detail")'>查询资产自定义项一致性校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项一致性校验规则启用/禁用", "operation")'>资产自定义项一致性校验规则启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项一致性校验失败告警", "operation")'>资产自定义项一致性校验失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产自定义项一致性校验结果统计", "analysis")'>资产自定义项一致性校验结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产自定义项一致性校验数据提取", "operation")'>资产自定义项一致性校验数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产自定义项一致性校验规则分析", "analysis")'>资产自定义项一致性校验规则分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产自定义项一致性校验规则分析结果展示", "detail")'>资产自定义项一致性校验规则分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("资产自定义项一致性校验规则分析结果下载", "export")'>资产自定义项一致性校验规则分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产自定义项一致性校验规则分析结果上报", "analysis")'>资产自定义项一致性校验规则分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
...
const initialRows = [
  ...
  },
  {
    "ID": 7,
    "规则编号": "RULE-89835",
    "规则名称": "资产自定义项一致性校验规则管理-告警模型",
    "规则类型": "风险评分",
    "启用状态": "待发布",
    "命中次数": 189,
    "更新时间": "2026-05-08 14:03:52"
  },
  {
    "ID": 8,
    "规则编号": "RULE-97754",
    "规则名称": "资产自定义项一致性校验规则管理-评分模型",
    "规则类型": "行为分析",
    "启用状态": "待审核",
    "命中次数": 245,
    "更新时间": "2026-05-08 15:21:37"
  },
  {
    "ID": 9,
    "规则编号": "RULE-105673",
...
const importTemplateRow = {
  ...
  "更新时间": "2026-05-08 16:08:25",
  "ID": "",
  "导入来源": "资产自定义项一致性校验规则管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage291",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产自定义项一致性校验规则管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产自定义项一致性校验规则管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产自定义项一致性校验规则管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "资产自定义项一致性校验规则管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "资产自定义项一致性校验规则管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产自定义项一致性校验规则管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "资产自定义项一致性校验规则管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "资产自定义项一致性校验规则管理" },
        { label: "一级模块", value: "模拟验证优化" },
        { label: "二级模块", value: "自定义填写一致性验证" },
      ];

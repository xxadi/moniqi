    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增虚拟资产纳管结果信息", "add")'>新增虚拟资产纳管结果信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改虚拟资产纳管结果信息", "edit")'>修改虚拟资产纳管结果信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除虚拟资产纳管结果信息", "delete")'>删除虚拟资产纳管结果信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看虚拟资产纳管结果信息", "detail")'>查看虚拟资产纳管结果信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("虚拟资产纳管结果数据提取", "operation")'>虚拟资产纳管结果数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("虚拟资产纳管结果数据识别", "operation")'>虚拟资产纳管结果数据识别</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("虚拟资产纳管结果清洗", "operation")'>虚拟资产纳管结果清洗</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("虚拟资产纳管结果分级", "operation")'>虚拟资产纳管结果分级</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("虚拟资产纳管结果实时计算", "operation")'>虚拟资产纳管结果实时计算</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("虚拟资产纳管结果聚合", "operation")'>虚拟资产纳管结果聚合</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("虚拟资产纳管结果信息导出", "export")'>虚拟资产纳管结果信息导出</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("虚拟资产纳管结果可视化展示", "detail")'>虚拟资产纳管结果可视化展示</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("虚拟资产纳管结果数据审查", "operation")'>虚拟资产纳管结果数据审查</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "虚拟资产纳管结果报表导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage289",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "虚拟资产纳管结果报表导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "虚拟资产纳管结果报表任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "虚拟资产纳管结果报表异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "虚拟资产纳管结果报表状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "虚拟资产纳管结果报表回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "虚拟资产纳管结果报表业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "虚拟资产纳管结果报表复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

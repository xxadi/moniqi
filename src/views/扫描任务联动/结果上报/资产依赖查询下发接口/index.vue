    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产依赖查询下发接口", "detail")'>资产依赖查询下发接口</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增资产依赖查询信息", "add")'>新增资产依赖查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改资产依赖查询信息", "edit")'>修改资产依赖查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除资产依赖查询信息", "delete")'>删除资产依赖查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产依赖查询信息查询", "detail")'>资产依赖查询信息查询</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产依赖查询信息统计", "detail")'>资产依赖查询信息统计</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("资产依赖查询数据导出", "export")'>资产依赖查询数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产依赖查询下发信息", "detail")'>资产依赖查询下发信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产依赖查询下发信息检索", "detail")'>资产依赖查询下发信息检索</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("资产依赖查询下发信息统计", "detail")'>资产依赖查询下发信息统计</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "下发时间": "2026-05-08 16:08:25",
  "ID": "",
  "导入来源": "资产依赖查询下发接口导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage129",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产依赖查询下发接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产依赖查询下发接口任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产依赖查询下发接口异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "资产依赖查询下发接口状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "资产依赖查询下发接口回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产依赖查询下发接口业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "资产依赖查询下发接口复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

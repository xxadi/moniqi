    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增探针数据解析规则", "add")'>新增探针数据解析规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改探针数据解析规则", "edit")'>修改探针数据解析规则</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除探针数据解析规则", "delete")'>删除探针数据解析规则</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询探针数据解析规则", "detail")'>查询探针数据解析规则</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("探针数据解析规则启用/禁用", "operation")'>探针数据解析规则启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("探针数据解析规则运行效果评估", "analysis")'>探针数据解析规则运行效果评估</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("探针数据解析规则分析", "analysis")'>探针数据解析规则分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("探针数据解析规则分析结果展示", "detail")'>探针数据解析规则分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("探针数据解析规则分析结果下载", "export")'>探针数据解析规则分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("探针数据解析规则分析结果上报", "analysis")'>探针数据解析规则分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "探针数据解析规则管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "探针数据解析规则管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "探针数据解析规则管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "探针数据解析规则管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

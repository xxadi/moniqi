    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询失败原因并展示说明", "detail")'>查询失败原因并展示说明</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加详细失败日志内容", "add")'>添加详细失败日志内容</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("保存失败类型与分析记录", "analysis")'>保存失败类型与分析记录</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("添加失败码与异常类型配置表", "edit")'>添加失败码与异常类型配置表</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增任务编号触发失败分析", "add")'>新增任务编号触发失败分析</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报失败原因采集", "operation")'>上报失败原因采集</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报失败原因信息研判", "operation")'>上报失败原因信息研判</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("上报失败原因关联分析", "analysis")'>上报失败原因关联分析</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报失败原因数据审查", "operation")'>上报失败原因数据审查</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报失败原因数据提取", "operation")'>上报失败原因数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看上报失败原因信息", "detail")'>查看上报失败原因信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("上报失败原因采集监控", "detail")'>上报失败原因采集监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("上报失败原因运行监控", "detail")'>上报失败原因运行监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("上报失败原因异常监控", "detail")'>上报失败原因异常监控</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报失败原因模型启用/禁用", "operation")'>上报失败原因模型启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("上报失败原因模型运行效果评估", "analysis")'>上报失败原因模型运行效果评估</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "上报失败原因分析状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "上报失败原因分析回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "上报失败原因分析业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "上报失败原因分析复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

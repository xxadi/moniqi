    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("历史上报数据采集", "operation")'>历史上报数据采集</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("历史上报数据信息研判", "operation")'>历史上报数据信息研判</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("历史上报数据关联分析", "analysis")'>历史上报数据关联分析</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("历史上报数据数据审查", "operation")'>历史上报数据数据审查</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("历史上报数据数据提取", "operation")'>历史上报数据数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看历史上报数据信息", "detail")'>查看历史上报数据信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("历史上报数据采集监控", "detail")'>历史上报数据采集监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("历史上报数据运行监控", "detail")'>历史上报数据运行监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("历史上报数据异常监控", "detail")'>历史上报数据异常监控</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "历史上报数据分析导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage167",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "历史上报数据分析导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "历史上报数据分析任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "历史上报数据分析异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "历史上报数据分析状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "历史上报数据分析回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "历史上报数据分析业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "历史上报数据分析复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IP地址资产上报字段库", "add")'>新增IP地址资产上报字段库</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("IP地址资产上报字段更新接口", "edit")'>IP地址资产上报字段更新接口</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("同步IP地址资产上报字段信息", "operation")'>同步IP地址资产上报字段信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("IP地址资产上报字段信息导出", "export")'>IP地址资产上报字段信息导出</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("IP地址资产上报字段修改记录查询", "edit")'>IP地址资产上报字段修改记录查询</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("IP地址资产上报状态查询信息", "detail")'>IP地址资产上报状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("IP地址资产上报字段异常监控", "detail")'>IP地址资产上报字段异常监控</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址资产上报字段反馈接口", "operation")'>IP地址资产上报字段反馈接口</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "IP地址资产上报字段库管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage109",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "IP地址资产上报字段库管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "IP地址资产上报字段库管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "IP地址资产上报字段库管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "IP地址资产上报字段库管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "IP地址资产上报字段库管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IP地址资产上报字段库管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "IP地址资产上报字段库管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "IP地址资产上报字段库管理" },

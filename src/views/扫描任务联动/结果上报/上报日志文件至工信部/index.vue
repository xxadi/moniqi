    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增数据上报指令", "add")'>新增数据上报指令</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载上报完成状态及返回码", "export")'>下载上报完成状态及返回码</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存上报响应与接口返回信息", "operation")'>保存上报响应与接口返回信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询接口调用日志", "detail")'>查询接口调用日志</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载工信部数据接口规范文档", "export")'>下载工信部数据接口规范文档</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "上报日志文件至工信部导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage153",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "上报日志文件至工信部导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "上报日志文件至工信部任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "上报日志文件至工信部异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "上报日志文件至工信部状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "上报日志文件至工信部回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "上报日志文件至工信部业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "上报日志文件至工信部复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产IP上报数据接口管理", "operation")'>资产IP上报数据接口管理</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("定义资产IP上报数据接口", "operation")'>定义资产IP上报数据接口</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("更新资产IP上报数据接口", "edit")'>更新资产IP上报数据接口</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("资产IP上报数据接口删除", "delete")'>资产IP上报数据接口删除</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询资产IP上报数据接口", "detail")'>查询资产IP上报数据接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产IP上报数据接口统计", "analysis")'>资产IP上报数据接口统计</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("资产IP上报数据接口数据导出", "export")'>资产IP上报数据接口数据导出</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "资产IP上报数据接口管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage123",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产IP上报数据接口管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产IP上报数据接口管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产IP上报数据接口管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "资产IP上报数据接口管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "资产IP上报数据接口管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产IP上报数据接口管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "资产IP上报数据接口管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

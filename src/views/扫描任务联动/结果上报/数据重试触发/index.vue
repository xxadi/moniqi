    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报任务结果重试", "operation")'>上报任务结果重试</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载重试结果或异常说明", "export")'>下载重试结果或异常说明</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("记录重试次数与重试时间", "operation")'>记录重试次数与重试时间</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询当前重试次数", "detail")'>查询当前重试次数</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("展示上报失败原因分析规则", "detail")'>展示上报失败原因分析规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "上报时间": "2026-05-08 09:35:44",
  "ID": "",
  "导入来源": "数据重试触发导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage158",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "数据重试触发导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "数据重试触发任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "数据重试触发异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "数据重试触发状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "数据重试触发回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "数据重试触发业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "数据重试触发复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

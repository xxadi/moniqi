    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("创建扫描任务触发日志", "add")'>创建扫描任务触发日志</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("生成的content.log文件", "add")'>生成的content.log文件</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存原始扫描日志数据", "operation")'>保存原始扫描日志数据</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询任务状态以判断是否生成日志", "add")'>查询任务状态以判断是否生成日志</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("关联扫描器的结果输出格式", "operation")'>关联扫描器的结果输出格式</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "上报时间": "2026-05-08 16:08:25",
  "ID": "",
  "导入来源": "生成扫描结果日志文件导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage148",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "生成扫描结果日志文件导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "生成扫描结果日志文件任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "生成扫描结果日志文件异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "生成扫描结果日志文件状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "生成扫描结果日志文件回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "生成扫描结果日志文件业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "生成扫描结果日志文件复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加content.log文件至日志表", "add")'>添加content.log文件至日志表</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存解析后的日志字段数据", "operation")'>保存解析后的日志字段数据</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询是否已存储过同一任务日志", "detail")'>查询是否已存储过同一任务日志</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加日志入库结果", "add")'>添加日志入库结果</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("新增日志字段模板配置", "edit")'>新增日志字段模板配置</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("上传日志后触发格式校验", "import")'>上传日志后触发格式校验</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加格式校验结果和提示信息", "add")'>添加格式校验结果和提示信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询校验规则及映射说明", "detail")'>查询校验规则及映射说明</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存校验错误记录日志", "operation")'>保存校验错误记录日志</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增国家标准字段模板", "add")'>新增国家标准字段模板</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "日志文件存储入库导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage149",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "日志文件存储入库导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "日志文件存储入库任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "日志文件存储入库异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "日志文件存储入库状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "日志文件存储入库回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "日志文件存储入库业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "日志文件存储入库复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

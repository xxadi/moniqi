    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令解析字段上报接口", "operation")'>指令解析字段上报接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令解析字段上报数据分析", "analysis")'>指令解析字段上报数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看指令解析字段上报信息", "detail")'>查看指令解析字段上报信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("指令解析字段上报数据导出", "export")'>指令解析字段上报数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令解析字段上报", "operation")'>指令解析字段上报</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增指令解析字段上报请求任务", "add")'>新增指令解析字段上报请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令解析字段上报结果反馈", "operation")'>指令解析字段上报结果反馈</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("导出指令解析字段上报任务执行状态与日志", "export")'>导出指令解析字段上报任务执行状态与日志</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控指令解析字段上报状态", "detail")'>监控指令解析字段上报状态</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令解析字段上报接口调用查询", "detail")'>指令解析字段上报接口调用查询</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令解析字段上报调用统计", "analysis")'>指令解析字段上报调用统计</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "入库状态": "待入库",
  "ID": "",
  "导入来源": "指令解析字段上报接口导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage19",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令解析字段上报接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令解析字段上报接口任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令解析字段上报接口异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "指令解析字段上报接口状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "指令解析字段上报接口回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令解析字段上报接口业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "指令解析字段上报接口复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

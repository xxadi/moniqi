    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP扫描指令下发接口", "operation")'>IP扫描指令下发接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP扫描指令下发数据分析", "analysis")'>IP扫描指令下发数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看IP扫描指令下发信息", "detail")'>查看IP扫描指令下发信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("IP扫描指令下发数据导出", "export")'>IP扫描指令下发数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP扫描指令下发", "operation")'>IP扫描指令下发</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("生成IP扫描指令下发请求任务", "add")'>生成IP扫描指令下发请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP扫描指令下发结果反馈", "operation")'>IP扫描指令下发结果反馈</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("导出IP扫描指令下发任务执行状态与日志", "export")'>导出IP扫描指令下发任务执行状态与日志</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控IP扫描指令下发状态", "detail")'>监控IP扫描指令下发状态</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IP扫描指令下发参数信息", "delete")'>删除IP扫描指令下发参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("IP扫描指令下发接口调用查询", "detail")'>IP扫描指令下发接口调用查询</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP扫描指令下发调用统计", "analysis")'>IP扫描指令下发调用统计</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "IP扫描指令下发接口状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "IP扫描指令下发接口回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IP扫描指令下发接口业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "IP扫描指令下发接口复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

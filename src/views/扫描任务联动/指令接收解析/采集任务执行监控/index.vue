    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加采集任务执行监控规则", "add")'>添加采集任务执行监控规则</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除采集任务执行监控规则", "delete")'>删除采集任务执行监控规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("编辑采集任务执行监控规则", "edit")'>编辑采集任务执行监控规则</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询采集任务执行监控规则", "add")'>查询采集任务执行监控规则</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集任务执行监控规则启用/禁用", "add")'>采集任务执行监控规则启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集任务执行监控规则运行效果评估", "add")'>采集任务执行监控规则运行效果评估</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集任务执行监控情况分析", "add")'>采集任务执行监控情况分析</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "采集任务执行监控导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "采集任务执行监控调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "采集任务执行监控异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "采集任务执行监控业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "采集任务执行监控复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

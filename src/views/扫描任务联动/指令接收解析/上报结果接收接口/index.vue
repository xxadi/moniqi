    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上报结果接收接口", "operation")'>上报结果接收接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("上报结果接收数据分析", "analysis")'>上报结果接收数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看上报结果接收信息", "detail")'>查看上报结果接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("上报结果接收数据导出", "export")'>上报结果接收数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增上报结果接收信息", "add")'>新增上报结果接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("输入上报结果接收请求任务", "operation")'>输入上报结果接收请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("输出上报结果接收反馈信息", "operation")'>输出上报结果接收反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控上报结果接收状态信息", "detail")'>监控上报结果接收状态信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("统计上报结果接收调用信息", "analysis")'>统计上报结果接收调用信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
const initialRows = [
  {
    "ID": 1,
    "接口编号": "API-522961-100-4894",
    "接口名称": "上报结果接收接口-部侧联动-01-522961",
    "对接系统": "集团资产平台-5229",
    "调用状态": "已完成-961",
    "平均耗时": "367ms-61",
    "最近调用": "2026-05-20 10:42:47"
  },
  {
    "ID": 2,
    "接口编号": "API-522961-101-2020",
    "接口名称": "区公司上报结果接收接口-数据上报-02-522961",
    "对接系统": "工信部对接平台-5229",
    "调用状态": "待复核-961",
    "平均耗时": "133ms-61",
    "最近调用": "2026-05-16 09:14:15"
  },
  {
    "ID": 3,
    "接口编号": "API-522961-102-8649",
    "接口名称": "上报结果接收接口-数据上报-03-522961",
    "对接系统": "CMDB资源库-5229",
    "调用状态": "待处理-961",
    "平均耗时": "673ms-61",
    "最近调用": "2026-05-11 15:35:31"
  },
  {
    "ID": 4,
    "接口编号": "API-522961-103-8117",
    "接口名称": "上报结果接收接口-部侧联动-04-522961",
    "对接系统": "集团资产平台-5229",
    "调用状态": "处理中-961",
    "平均耗时": "397ms-61",
    "最近调用": "2026-05-21 11:05:34"
  },
  {
    "ID": 5,
    "接口编号": "API-522961-104-8507",
    "接口名称": "区公司上报结果接收接口-异常复核-05-522961",
    "对接系统": "工信部对接平台-5229",
    "调用状态": "异常-961",
    "平均耗时": "130ms-61",
    "最近调用": "2026-05-16 15:51:27"
  },
  {
    "ID": 6,
    "接口编号": "API-522961-105-1749",
    "接口名称": "上报结果接收接口-资产纳管-06-522961",
    "对接系统": "调度中心-5229",
    "调用状态": "已同步-961",
    "平均耗时": "278ms-61",
    "最近调用": "2026-05-15 08:42:47"
  },
  {
    "ID": 7,
    "接口编号": "API-522961-106-9714",
    "接口名称": "上报结果接收接口-数据上报-07-522961",
    "对接系统": "工信部对接平台-5229",
    "调用状态": "已同步-961",
    "平均耗时": "203ms-61",
    "最近调用": "2026-05-16 13:01:23"
  },
  {
    "ID": 8,
    "接口编号": "API-522961-107-6722",
    "接口名称": "上报结果接收接口-扩容核查-08-522961",
    "对接系统": "CMDB资源库-5229",
    "调用状态": "异常-961",
    "平均耗时": "262ms-61",
    "最近调用": "2026-05-10 15:21:09"
  },
  {
    "ID": 9,
    "接口编号": "API-522961-108-8922",
    "接口名称": "上报结果接收接口-异常复核-09-522961",
    "对接系统": "调度中心-5229",
    "调用状态": "处理中-961",
    "平均耗时": "576ms-61",
    "最近调用": "2026-05-13 12:47:40"
  },
  {
    "ID": 10,
    "接口编号": "API-522961-109-4661",
    "接口名称": "上报结果接收接口-异常复核-10-522961",
    "对接系统": "资产安全平台-5229",
    "调用状态": "异常-961",
    "平均耗时": "572ms-61",
    "最近调用": "2026-05-17 13:46:43"
  }
];
const tableColumns = [
  {
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "上报结果接收接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "上报结果接收接口调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "上报结果接收接口异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "上报结果接收接口业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "上报结果接收接口复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("发起部侧结果上报流程", "add")'>发起部侧结果上报流程</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加部侧结果上报指派信息", "add")'>添加部侧结果上报指派信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加部侧结果上报流程节点", "add")'>添加部侧结果上报流程节点</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("编辑部侧结果上报流程节点信息", "edit")'>编辑部侧结果上报流程节点信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("部侧结果上报流程节点删除", "delete")'>部侧结果上报流程节点删除</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("部侧结果上报流程节点查询", "detail")'>部侧结果上报流程节点查询</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("上传部侧结果上报附件", "import")'>上传部侧结果上报附件</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("部侧结果上报流程图查看", "detail")'>部侧结果上报流程图查看</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("部侧结果上报流程通知", "operation")'>部侧结果上报流程通知</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("部侧结果上报结果逐级审批", "edit")'>部侧结果上报结果逐级审批</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("部侧结果上报结果拒绝和驳回", "operation")'>部侧结果上报结果拒绝和驳回</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "部侧结果上报流程管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "部侧结果上报流程管理调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "部侧结果上报流程管理异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "部侧结果上报流程管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "部侧结果上报流程管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

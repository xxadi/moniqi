    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("往期数据查看", "detail")'>往期数据查看</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果与往期数据信息研判", "add")'>采集结果与往期数据信息研判</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果与往期数据关联分析", "add")'>采集结果与往期数据关联分析</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果与往期数据数据审查", "add")'>采集结果与往期数据数据审查</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果与往期数据数据提取", "add")'>采集结果与往期数据数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查看采集结果与往期数据信息", "add")'>查看采集结果与往期数据信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果与往期数据采集监控", "add")'>采集结果与往期数据采集监控</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果与往期数据异常监控", "add")'>采集结果与往期数据异常监控</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "采集结果与往期数据分析导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "采集结果与往期数据分析调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "采集结果与往期数据分析异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "采集结果与往期数据分析业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "采集结果与往期数据分析复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加指令版本兼容性识别信息", "add")'>添加指令版本兼容性识别信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("调整指令版本兼容性识别信息", "edit")'>调整指令版本兼容性识别信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("移除指令版本兼容性识别信息", "delete")'>移除指令版本兼容性识别信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询指令版本兼容性识别", "detail")'>查询指令版本兼容性识别</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令版本兼容性识别统计", "analysis")'>指令版本兼容性识别统计</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("指令版本兼容性识别下载", "export")'>指令版本兼容性识别下载</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "更新时间": "2026-05-08 16:08:25",
  "ID": "",
  "导入来源": "指令版本兼容性识别管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage10",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令版本兼容性识别管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令版本兼容性识别管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令版本兼容性识别管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "指令版本兼容性识别管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "指令版本兼容性识别管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令版本兼容性识别管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "指令版本兼容性识别管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

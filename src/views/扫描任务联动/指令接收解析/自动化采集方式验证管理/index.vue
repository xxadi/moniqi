    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增自动化采集方式验证", "add")'>新增自动化采集方式验证</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改自动化采集方式验证", "edit")'>修改自动化采集方式验证</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除自动化采集方式验证", "delete")'>删除自动化采集方式验证</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询自动化采集方式验证", "add")'>查询自动化采集方式验证</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("自动化采集方式验证启用/禁用", "add")'>自动化采集方式验证启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("自动化采集方式验证运行效果评估", "add")'>自动化采集方式验证运行效果评估</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("自动化采集方式验证分析", "add")'>自动化采集方式验证分析</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("自动化采集方式验证分析结果展示", "add")'>自动化采集方式验证分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("自动化采集方式验证分析结果下载", "export")'>自动化采集方式验证分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("自动化采集方式验证分析结果上报", "add")'>自动化采集方式验证分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "自动化采集方式验证管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "自动化采集方式验证管理调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "自动化采集方式验证管理异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "自动化采集方式验证管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "自动化采集方式验证管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

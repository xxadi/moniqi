    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IP扫描指令接收信息", "add")'>新增IP扫描指令接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改IP扫描指令接收信息", "edit")'>修改IP扫描指令接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IP扫描指令接收信息", "delete")'>删除IP扫描指令接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询IP扫描指令接收信息", "detail")'>查询IP扫描指令接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("IP扫描指令接收监控", "detail")'>IP扫描指令接收监控</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP扫描指令接收告警", "operation")'>IP扫描指令接收告警</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("IP扫描指令接收查看", "detail")'>IP扫描指令接收查看</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP扫描指令接收分析", "analysis")'>IP扫描指令接收分析</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("IP扫描指令接收信息导出", "export")'>IP扫描指令接收信息导出</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP扫描指令接收", "operation")'>IP扫描指令接收</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IP扫描指令接收请求任务", "add")'>新增IP扫描指令接收请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP扫描指令接收结果反馈", "operation")'>IP扫描指令接收结果反馈</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("导出IP扫描指令接收任务执行状态与日志", "export")'>导出IP扫描指令接收任务执行状态与日志</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改IP扫描指令接收参数信息", "edit")'>修改IP扫描指令接收参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IP扫描指令接收参数信息", "delete")'>删除IP扫描指令接收参数信息</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "IP扫描指令接收管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "IP扫描指令接收管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IP扫描指令接收管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "IP扫描指令接收管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增扫描参数结构识别任务", "add")'>新增扫描参数结构识别任务</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除扫描参数结构识别任务", "delete")'>删除扫描参数结构识别任务</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改扫描参数结构识别任务", "edit")'>修改扫描参数结构识别任务</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询扫描参数结构识别任务", "detail")'>查询扫描参数结构识别任务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("扫描参数结构识别任务启用/禁用", "operation")'>扫描参数结构识别任务启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("扫描参数结构识别失败告警", "operation")'>扫描参数结构识别失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描参数结构识别结果统计", "analysis")'>扫描参数结构识别结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("扫描参数结构识别数据提取", "operation")'>扫描参数结构识别数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描参数结构识别运行分析", "analysis")'>扫描参数结构识别运行分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("扫描参数结构识别分析结果展示", "detail")'>扫描参数结构识别分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("扫描参数结构识别分析结果下载", "export")'>扫描参数结构识别分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描参数结构识别分析结果上报", "analysis")'>扫描参数结构识别分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 09:35:44",
  "ID": "",
  "导入来源": "扫描参数结构识别任务管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage25",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "扫描参数结构识别任务管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "扫描参数结构识别任务管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "扫描参数结构识别任务管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "扫描参数结构识别任务管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "扫描参数结构识别任务管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "扫描参数结构识别任务管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "扫描参数结构识别任务管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

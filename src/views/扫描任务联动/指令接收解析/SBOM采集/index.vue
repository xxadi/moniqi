    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增SBOM采集", "add")'>新增SBOM采集</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除SBOM采集", "delete")'>删除SBOM采集</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改SBOM采集", "edit")'>修改SBOM采集</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询SBOM采集", "add")'>查询SBOM采集</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("SBOM采集启用/禁用", "add")'>SBOM采集启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("SBOM采集运行效果评估", "add")'>SBOM采集运行效果评估</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("SBOM采集分析", "add")'>SBOM采集分析</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("SBOM采集分析结果展示", "add")'>SBOM采集分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("SBOM采集分析结果下载", "export")'>SBOM采集分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("SBOM采集分析结果上报", "add")'>SBOM采集分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "SBOM采集导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "SBOM采集调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "SBOM采集异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "SBOM采集业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "SBOM采集复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

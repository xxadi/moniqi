    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集器存活审查", "add")'>采集器存活审查</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集器存活审查结果查看", "add")'>采集器存活审查结果查看</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("采集器存活审查结果删除", "delete")'>采集器存活审查结果删除</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集器存活审查结果统计", "add")'>采集器存活审查结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集器存活审查结果推送", "add")'>采集器存活审查结果推送</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集器存活审查结果归档", "add")'>采集器存活审查结果归档</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "采集器存活审查管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "采集器存活审查管理调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "采集器存活审查管理异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "采集器存活审查管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "采集器存活审查管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

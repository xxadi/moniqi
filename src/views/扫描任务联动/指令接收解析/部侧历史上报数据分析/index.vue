    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("部侧历史上报数据查看", "detail")'>部侧历史上报数据查看</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("部侧历史上报数据信息研判", "operation")'>部侧历史上报数据信息研判</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("部侧历史上报数据关联分析", "analysis")'>部侧历史上报数据关联分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("部侧历史上报数据数据审查", "detail")'>部侧历史上报数据数据审查</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("部侧历史上报数据数据提取", "operation")'>部侧历史上报数据数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看部侧历史上报数据信息", "detail")'>查看部侧历史上报数据信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("部侧历史上报数据采集监控", "add")'>部侧历史上报数据采集监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("部侧历史上报数据运行监控", "detail")'>部侧历史上报数据运行监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("部侧历史上报数据异常监控", "detail")'>部侧历史上报数据异常监控</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("部侧历史上报数据模型启用/禁用", "operation")'>部侧历史上报数据模型启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("部侧历史上报数据模型运行效果评估", "analysis")'>部侧历史上报数据模型运行效果评估</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "部侧历史上报数据分析导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "部侧历史上报数据分析调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "部侧历史上报数据分析异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "部侧历史上报数据分析业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "部侧历史上报数据分析复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

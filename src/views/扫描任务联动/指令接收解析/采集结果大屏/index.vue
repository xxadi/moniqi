    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果折线图展示", "add")'>采集结果折线图展示</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查看采集结果详情", "add")'>查看采集结果详情</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果TOP10展示", "add")'>采集结果TOP10展示</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果百分比展示", "add")'>采集结果百分比展示</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果饼图展示", "add")'>采集结果饼图展示</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果柱状图展示", "add")'>采集结果柱状图展示</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果数据展示", "add")'>采集结果数据展示</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("采集结果热力图展示", "add")'>采集结果热力图展示</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "采集结果大屏导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "采集结果大屏调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "采集结果大屏异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "采集结果大屏业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "采集结果大屏复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

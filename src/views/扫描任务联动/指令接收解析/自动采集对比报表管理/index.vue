    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增自动采集对比报表信息", "add")'>新增自动采集对比报表信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改自动采集对比报表信息", "edit")'>修改自动采集对比报表信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除自动采集对比报表信息", "delete")'>删除自动采集对比报表信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询自动采集对比报表列表", "add")'>查询自动采集对比报表列表</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询自动采集对比报表详情", "add")'>查询自动采集对比报表详情</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("批量导入自动采集对比报表信息", "import")'>批量导入自动采集对比报表信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("自动采集对比报表信息导出", "export")'>自动采集对比报表信息导出</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "自动采集对比报表管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "自动采集对比报表管理调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "自动采集对比报表管理异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "自动采集对比报表管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "自动采集对比报表管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令采集日志采集", "operation")'>指令采集日志采集</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令采集日志信息研判", "operation")'>指令采集日志信息研判</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令采集日志关联分析", "analysis")'>指令采集日志关联分析</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令采集日志数据审查", "operation")'>指令采集日志数据审查</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令采集日志数据提取", "operation")'>指令采集日志数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看指令采集日志信息", "detail")'>查看指令采集日志信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令采集日志采集监控", "detail")'>指令采集日志采集监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令采集日志运行监控", "detail")'>指令采集日志运行监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令采集日志异常监控", "detail")'>指令采集日志异常监控</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令采集日志模型启用/禁用", "operation")'>指令采集日志模型启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令采集日志模型运行效果评估", "analysis")'>指令采集日志模型运行效果评估</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "处理状态": "已归档",
  "ID": "",
  "导入来源": "指令采集日志分析导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage40",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令采集日志分析导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令采集日志分析任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令采集日志分析异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "指令采集日志分析状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "指令采集日志分析回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令采集日志分析业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "指令采集日志分析复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("添加内网采集指令文件信息", "import")'>添加内网采集指令文件信息</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("编辑内网采集指令文件信息", "import")'>编辑内网采集指令文件信息</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("删除内网采集指令文件信息", "import")'>删除内网采集指令文件信息</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("查询内网采集指令文件列表", "import")'>查询内网采集指令文件列表</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("查询内网采集指令文件详情", "import")'>查询内网采集指令文件详情</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("批量导入内网采集指令文件信息", "import")'>批量导入内网采集指令文件信息</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("内网采集指令文件信息导出", "import")'>内网采集指令文件信息导出</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "内网采集指令文件管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "内网采集指令文件管理调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "内网采集指令文件管理异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "内网采集指令文件管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "内网采集指令文件管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

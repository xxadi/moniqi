    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集结果上报接口", "add")'>内网采集结果上报接口</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集结果上报数据分析", "add")'>内网采集结果上报数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查看内网采集结果上报信息", "add")'>查看内网采集结果上报信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("内网采集结果上报数据导出", "export")'>内网采集结果上报数据导出</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加内网采集结果上报信息", "add")'>添加内网采集结果上报信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("输入内网采集结果上报请求任务", "add")'>输入内网采集结果上报请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("输出内网采集结果上报反馈信息", "add")'>输出内网采集结果上报反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("监控内网采集结果上报状态信息", "add")'>监控内网采集结果上报状态信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("统计内网采集结果上报调用信息", "add")'>统计内网采集结果上报调用信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
const initialRows = [
  {
    "ID": 1,
    "接口编号": "API-165557-100-1405",
    "接口名称": "区公司内网采集结果上报接口-数据上报-01-165557",
    "对接系统": "调度中心-1655",
    "调用状态": "异常-557",
    "平均耗时": "699ms-57",
    "最近调用": "2026-05-24 12:18:28"
  },
  {
    "ID": 2,
    "接口编号": "API-165557-101-7092",
    "接口名称": "内网采集结果上报接口-扩容核查-02-165557",
    "对接系统": "CMDB资源库-1655",
    "调用状态": "处理中-557",
    "平均耗时": "654ms-57",
    "最近调用": "2026-05-25 17:36:35"
  },
  {
    "ID": 3,
    "接口编号": "API-165557-102-4818",
    "接口名称": "内网采集结果上报接口-部侧联动-03-165557",
    "对接系统": "资产安全平台-1655",
    "调用状态": "已同步-557",
    "平均耗时": "630ms-57",
    "最近调用": "2026-05-23 08:44:44"
  },
  {
    "ID": 4,
    "接口编号": "API-165557-103-1015",
    "接口名称": "内网采集结果上报接口-扩容核查-04-165557",
    "对接系统": "资产安全平台-1655",
    "调用状态": "异常-557",
    "平均耗时": "314ms-57",
    "最近调用": "2026-05-11 09:13:44"
  },
  {
    "ID": 5,
    "接口编号": "API-165557-104-7164",
    "接口名称": "内网采集结果上报接口-资产纳管-05-165557",
    "对接系统": "资产安全平台-1655",
    "调用状态": "已同步-557",
    "平均耗时": "234ms-57",
    "最近调用": "2026-05-08 11:48:50"
  },
  {
    "ID": 6,
    "接口编号": "API-165557-105-2120",
    "接口名称": "内网采集结果上报接口-资产纳管-06-165557",
    "对接系统": "工信部对接平台-1655",
    "调用状态": "异常-557",
    "平均耗时": "488ms-57",
    "最近调用": "2026-05-19 12:22:30"
  },
  {
    "ID": 7,
    "接口编号": "API-165557-106-5007",
    "接口名称": "内网采集结果上报接口-数据上报-07-165557",
    "对接系统": "调度中心-1655",
    "调用状态": "已同步-557",
    "平均耗时": "277ms-57",
    "最近调用": "2026-05-10 10:35:24"
  },
  {
    "ID": 8,
    "接口编号": "API-165557-107-4126",
    "接口名称": "内网采集结果上报接口-资产纳管-08-165557",
    "对接系统": "工信部对接平台-1655",
    "调用状态": "已完成-557",
    "平均耗时": "393ms-57",
    "最近调用": "2026-05-14 16:02:30"
  },
  {
    "ID": 9,
    "接口编号": "API-165557-108-2582",
    "接口名称": "内网采集结果上报接口-资产纳管-09-165557",
    "对接系统": "调度中心-1655",
    "调用状态": "处理中-557",
    "平均耗时": "288ms-57",
    "最近调用": "2026-05-09 14:09:51"
  },
  {
    "ID": 10,
    "接口编号": "API-165557-109-4558",
    "接口名称": "内网采集结果上报接口-自动采集-10-165557",
    "对接系统": "集团资产平台-1655",
    "调用状态": "已完成-557",
    "平均耗时": "108ms-57",
    "最近调用": "2026-05-16 15:41:33"
  }
];
const tableColumns = [
  {
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "内网采集结果上报接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "内网采集结果上报接口调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "内网采集结果上报接口异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "内网采集结果上报接口业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "内网采集结果上报接口复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

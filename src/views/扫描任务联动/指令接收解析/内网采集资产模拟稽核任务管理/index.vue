    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增内网采集资产模拟稽核任务", "add")'>新增内网采集资产模拟稽核任务</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除内网采集资产模拟稽核任务", "delete")'>删除内网采集资产模拟稽核任务</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改内网采集资产模拟稽核任务", "edit")'>修改内网采集资产模拟稽核任务</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("查询内网采集资产模拟稽核任务", "add")'>查询内网采集资产模拟稽核任务</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集资产模拟稽核任务启用/禁用", "add")'>内网采集资产模拟稽核任务启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集资产模拟稽核告警", "add")'>内网采集资产模拟稽核告警</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集资产模拟稽核结果统计", "add")'>内网采集资产模拟稽核结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集资产模拟稽核数据提取", "add")'>内网采集资产模拟稽核数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集资产模拟稽核运行分析", "add")'>内网采集资产模拟稽核运行分析</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集资产模拟稽核分析结果展示", "add")'>内网采集资产模拟稽核分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("内网采集资产模拟稽核分析结果下载", "export")'>内网采集资产模拟稽核分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("内网采集资产模拟稽核分析结果上报", "add")'>内网采集资产模拟稽核分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      exportXLSX(this.buildExportRows(functionName), functionName || "内网采集资产模拟稽核任务管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志|监控/.test(functionName)) {
        return [
          ...
          { 名称: "内网采集资产模拟稽核任务管理调度任务", 状态: "执行中", 日志: "等待节点返回执行结果" },
          { 名称: "内网采集资产模拟稽核任务管理异常检查", 状态: "已告警", 日志: "发现超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "内网采集资产模拟稽核任务管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "内网采集资产模拟稽核任务管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "三级模块", value: "内网采集资产模拟稽核任务管理" },

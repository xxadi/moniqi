    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IPv4快速扫描任务", "add")'>新增IPv4快速扫描任务</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IPv4快速扫描任务", "delete")'>删除IPv4快速扫描任务</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改IPv4快速扫描任务", "edit")'>修改IPv4快速扫描任务</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询IPv4快速扫描任务", "detail")'>查询IPv4快速扫描任务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IPv4快速扫描任务启用/禁用", "operation")'>IPv4快速扫描任务启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IPv4快速扫描失败告警", "operation")'>IPv4快速扫描失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IPv4快速扫描结果统计", "analysis")'>IPv4快速扫描结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IPv4快速扫描数据提取", "operation")'>IPv4快速扫描数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IPv4快速扫描运行分析", "analysis")'>IPv4快速扫描运行分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("IPv4快速扫描分析结果展示", "detail")'>IPv4快速扫描分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("IPv4快速扫描分析结果下载", "export")'>IPv4快速扫描分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IPv4快速扫描分析结果上报", "analysis")'>IPv4快速扫描分析结果上报</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IPv4快速扫描推送", "operation")'>IPv4快速扫描推送</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "IPv4快速扫描任务管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage101",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "IPv4快速扫描任务管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "IPv4快速扫描任务管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "IPv4快速扫描任务管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "IPv4快速扫描任务管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "IPv4快速扫描任务管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IPv4快速扫描任务管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "IPv4快速扫描任务管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

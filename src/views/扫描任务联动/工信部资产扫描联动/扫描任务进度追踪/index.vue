    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("扫描任务状态运行监控", "detail")'>扫描任务状态运行监控</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("扫描任务状态运行告警", "operation")'>扫描任务状态运行告警</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("扫描任务状态结果稽核", "operation")'>扫描任务状态结果稽核</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("批量删除扫描任务状态信息", "delete")'>批量删除扫描任务状态信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务状态告警数据分析", "analysis")'>扫描任务状态告警数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务状态告警数据分析结果推送", "analysis")'>扫描任务状态告警数据分析结果推送</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("跟踪任务执行中状态", "operation")'>跟踪任务执行中状态</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存执行过程日志", "operation")'>保存执行过程日志</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("查询进度更新记录", "edit")'>查询进度更新记录</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("推送进度快照", "operation")'>推送进度快照</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看进度详情", "detail")'>查看进度详情</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "扫描任务进度追踪导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage72",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "扫描任务进度追踪导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "扫描任务进度追踪任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "扫描任务进度追踪异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "扫描任务进度追踪状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "扫描任务进度追踪回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "扫描任务进度追踪业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "扫描任务进度追踪复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令接收状态对账", "operation")'>指令接收状态对账</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令处理任务调度注册", "operation")'>指令处理任务调度注册</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令响应流程执行", "operation")'>指令响应流程执行</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令处理日志记录", "operation")'>指令处理日志记录</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("响应任务状态跟踪", "operation")'>响应任务状态跟踪</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令处理过程监控", "detail")'>指令处理过程监控</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "入库状态": "待入库",
  "ID": "",
  "导入来源": "指令接收状态导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage54",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令接收状态导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令接收状态任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令接收状态异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "指令接收状态状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "指令接收状态回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令接收状态业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "指令接收状态复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

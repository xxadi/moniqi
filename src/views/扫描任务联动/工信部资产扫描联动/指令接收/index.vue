    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("指令接收登记", "add")'>指令接收登记</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令报文数据解析", "operation")'>指令报文数据解析</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令信息结构合法性校验", "operation")'>指令信息结构合法性校验</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令字段映射与标准转换", "operation")'>指令字段映射与标准转换</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("接收到指令的数据落库处理", "operation")'>接收到指令的数据落库处理</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令接收导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令接收任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令接收异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "指令接收状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "指令接收回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令接收业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "指令接收复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

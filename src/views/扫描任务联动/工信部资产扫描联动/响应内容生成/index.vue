    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("响应内容保存", "operation")'>响应内容保存</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("企业处理情况反馈上报", "operation")'>企业处理情况反馈上报</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令任务状态综合看板展示", "detail")'>指令任务状态综合看板展示</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令生命周期全流程跟踪", "operation")'>指令生命周期全流程跟踪</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("跨平台指令传输安全校验", "operation")'>跨平台指令传输安全校验</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 15:21:37",
  "ID": "",
  "导入来源": "响应内容生成导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage55",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "响应内容生成导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "响应内容生成任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "响应内容生成异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "响应内容生成状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "响应内容生成回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "响应内容生成业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "响应内容生成复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("定义IPv4指纹识别信息", "operation")'>定义IPv4指纹识别信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改IPv4指纹识别信息", "edit")'>修改IPv4指纹识别信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IPv4指纹识别信息", "delete")'>删除IPv4指纹识别信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IPv4指纹识别数据获取", "operation")'>IPv4指纹识别数据获取</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IPv4指纹识别数据锁定", "operation")'>IPv4指纹识别数据锁定</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IPv4指纹识别数据激活", "operation")'>IPv4指纹识别数据激活</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询IPv4指纹识别报告", "detail")'>查询IPv4指纹识别报告</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IPv4指纹识别状态查询信息", "add")'>新增IPv4指纹识别状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改IPv4指纹识别状态查询信息", "edit")'>修改IPv4指纹识别状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IPv4指纹识别状态查询信息", "delete")'>删除IPv4指纹识别状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IPv4指纹识别参数信息", "add")'>新增IPv4指纹识别参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改IPv4指纹识别参数信息", "edit")'>修改IPv4指纹识别参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IPv4指纹识别参数信息", "delete")'>删除IPv4指纹识别参数信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IPv4指纹识别错误处理信息", "add")'>新增IPv4指纹识别错误处理信息</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IPv4指纹识别错误事件通知规则", "add")'>新增IPv4指纹识别错误事件通知规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 09:35:44",
  "ID": "",
  "导入来源": "IPv4指纹识别管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage95",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "IPv4指纹识别管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "IPv4指纹识别管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "IPv4指纹识别管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "IPv4指纹识别管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "IPv4指纹识别管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IPv4指纹识别管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "IPv4指纹识别管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

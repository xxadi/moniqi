    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增扫描任务动态分配规则", "add")'>新增扫描任务动态分配规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改扫描任务动态分配规则", "edit")'>修改扫描任务动态分配规则</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除扫描任务动态分配规则", "delete")'>删除扫描任务动态分配规则</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询扫描任务动态分配规则", "detail")'>查询扫描任务动态分配规则</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("扫描任务动态分配规则启用/禁用", "operation")'>扫描任务动态分配规则启用/禁用</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务动态分配规则运行效果评估", "analysis")'>扫描任务动态分配规则运行效果评估</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务动态分配规则分析", "analysis")'>扫描任务动态分配规则分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("扫描任务动态分配规则分析结果展示", "detail")'>扫描任务动态分配规则分析结果展示</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("扫描任务动态分配规则分析结果下载", "export")'>扫描任务动态分配规则分析结果下载</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务动态分配规则分析结果上报", "analysis")'>扫描任务动态分配规则分析结果上报</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    buildExportRows(functionName) {
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "扫描任务动态分配规则管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "扫描任务动态分配规则管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "扫描任务动态分配规则管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "扫描任务动态分配规则管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

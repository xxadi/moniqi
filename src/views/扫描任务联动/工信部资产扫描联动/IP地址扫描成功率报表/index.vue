    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增IP地址扫描成功率信息", "add")'>新增IP地址扫描成功率信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改IP地址扫描成功率信息", "edit")'>修改IP地址扫描成功率信息</el-button>
          <el-button plain type="primary" icon="el-icon-delete" @click='handleFunction("删除IP地址扫描成功率信息", "delete")'>删除IP地址扫描成功率信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看IP地址扫描成功率信息", "detail")'>查看IP地址扫描成功率信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址扫描成功率数据提取", "operation")'>IP地址扫描成功率数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址扫描成功率数据识别", "operation")'>IP地址扫描成功率数据识别</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址扫描成功率清洗", "operation")'>IP地址扫描成功率清洗</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址扫描成功率分级", "operation")'>IP地址扫描成功率分级</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址扫描成功率实时计算", "operation")'>IP地址扫描成功率实时计算</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址扫描成功率聚合", "operation")'>IP地址扫描成功率聚合</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("IP地址扫描成功率信息导出", "export")'>IP地址扫描成功率信息导出</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("IP地址扫描成功率可视化展示", "detail")'>IP地址扫描成功率可视化展示</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP地址扫描成功率数据审查", "operation")'>IP地址扫描成功率数据审查</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 15:21:37",
  "ID": "",
  "导入来源": "IP地址扫描成功率报表导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage92",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "IP地址扫描成功率报表导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "IP地址扫描成功率报表任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "IP地址扫描成功率报表异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "IP地址扫描成功率报表状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "IP地址扫描成功率报表回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IP地址扫描成功率报表业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "IP地址扫描成功率报表复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

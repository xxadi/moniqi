    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("指令下发操作权限新增", "add")'>指令下发操作权限新增</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("指令处理能力适配规则创建", "add")'>指令处理能力适配规则创建</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("指令解析组件版本修改", "edit")'>指令解析组件版本修改</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("历史指令下发与反馈查询", "detail")'>历史指令下发与反馈查询</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "指令下发操作权限控制导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage56",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令下发操作权限控制导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令下发操作权限控制任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令下发操作权限控制异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "指令下发操作权限控制状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "指令下发操作权限控制回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令下发操作权限控制业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "指令下发操作权限控制复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("创建唯一任务编号", "add")'>创建唯一任务编号</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改任务编号索引", "edit")'>修改任务编号索引</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询已分配编号", "detail")'>查询已分配编号</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("新增主键编码规则", "add")'>新增主键编码规则</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("导出扫描任务ID生成机制信息", "export")'>导出扫描任务ID生成机制信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "计划时间": "2026-05-08 15:21:37",
  "ID": "",
  "导入来源": "扫描任务ID生成机制导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage64",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "扫描任务ID生成机制导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "扫描任务ID生成机制任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "扫描任务ID生成机制异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "扫描任务ID生成机制状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "扫描任务ID生成机制回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "扫描任务ID生成机制业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "扫描任务ID生成机制复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

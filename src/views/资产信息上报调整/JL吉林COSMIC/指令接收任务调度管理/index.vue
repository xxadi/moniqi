    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增指令接收任务调度信息", "form")'>新增指令接收任务调度信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令接收任务调度结果监控", "monitor")'>指令接收任务调度结果监控</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("指令接收任务调度结果监控告警", "monitor_alert")'>指令接收任务调度结果监控告警</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("指令接收任务调度结果稽核", "audit")'>指令接收任务调度结果稽核</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令接收任务调度结果统计", "statistics")'>指令接收任务调度结果统计</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令接收任务调度数据采集", "collection")'>指令接收任务调度数据采集</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令接收任务调度数据转换", "transform")'>指令接收任务调度数据转换</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令接收任务调度数据预处理", "preprocess")'>指令接收任务调度数据预处理</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令接收任务调度管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令接收任务调度管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令接收任务调度管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令接收任务调度管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "指令接收任务调度管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("下发处置工单信息", "api")'>下发处置工单信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询处置下发工单列表", "detail")'>查询处置下发工单列表</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='openImport("批量导入处置下发工单")'>批量导入处置下发工单</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("处置下发工单监控", "monitor")'>处置下发工单监控</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("处置下发工单监控告警", "monitor_alert")'>处置下发工单监控告警</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("处置下发流程图查看", "flowchart")'>处置下发流程图查看</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("处置下发流程通知", "flow_notify")'>处置下发流程通知</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("处置下发结果逐级审批", "approval")'>处置下发结果逐级审批</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("处置下发结果拒绝和驳回", "reject")'>处置下发结果拒绝和驳回</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "处置下发工单管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "处置下发工单管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "处置下发工单管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "处置下发工单管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "处置下发工单管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

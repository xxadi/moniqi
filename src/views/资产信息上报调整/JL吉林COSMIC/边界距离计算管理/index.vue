    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增边界距离计算信息", "form")'>新增边界距离计算信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询边界距离计算列表", "detail")'>查询边界距离计算列表</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='openImport("批量导入边界距离计算信息")'>批量导入边界距离计算信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  "指令来源": "指令来源导入值",
  "指令状态": "已完成",
  "下发时间": "2026-06-29 02:55:42",
  "指令编号": "IMP-742007",
  "当前环节": "当前环节导入值",
  "探针节点": "探针节点导入值",
  "成功率": "成功率导入值",
  "IP地址": "IP地址导入值",
  "导入来源": "边界距离计算管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "边界距离计算管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "边界距离计算管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "边界距离计算管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "边界距离计算管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "边界距离计算管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

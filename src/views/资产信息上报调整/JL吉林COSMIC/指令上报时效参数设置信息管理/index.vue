    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增时效参数设置", "form")'>新增时效参数设置</el-button>
        </div>
      </div>
      <cs-pagetable
...
const tableColumns = [
  ...
  { prop: "参数编号", label: "参数编号", type: "text", search: true, showTooltip: true },
  { prop: "参数名称", label: "参数名称", type: "text", search: true, showTooltip: true },
  { prop: "参数类型", label: "参数类型", type: "select", search: true, showTooltip: true, options: ["超时时间", "重试次数", "上报频率", "阈值设置", "通道配置"] },
  { prop: "参数值", label: "参数值", type: "text", search: false, showTooltip: true },
  { prop: "默认值", label: "默认值", type: "text", search: false, showTooltip: true },
  { prop: "生效状态", label: "生效状态", type: "select", search: true, showTooltip: true, options: ["已生效", "未生效", "已过期", "已禁用"] },
  { prop: "修改人", label: "修改人", type: "text", search: false, showTooltip: true },
  { prop: "修改时间", label: "修改时间", type: "text", search: false, showTooltip: true, width: 170 },
...
const paramTypePool = ["超时时间", "重试次数", "上报频率", "阈值设置", "通道配置"];
const paramValuePool = ["30s", "3", "5min", "80%", "SFTP通道"];
const defaultValPool = ["60s", "5", "10min", "90%", "HTTP通道"];
const effectPool = ["已生效", "未生效", "已过期", "已禁用"];
...
function genInitialRows(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    row["ID"] = i + 1;
    row["参数编号"] = "PRM-" + String(2024000 + i);
    row["参数名称"] = pick(["超时时间参数", "重试次数参数", "上报频率参数", "阈值参数", "通道配置参数"], i);
    row["参数类型"] = paramTypePool[i % paramTypePool.length];
    row["参数值"] = paramValuePool[i % paramValuePool.length];
    row["默认值"] = defaultValPool[i % defaultValPool.length];
    row["生效状态"] = effectPool[i % effectPool.length];
    row["修改人"] = namePool[i % namePool.length];
    row["修改时间"] = genRecentTime(i);

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增content文件生成任务", "form")'>新增content文件生成任务</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询content文件生成任务", "detail")'>查询content文件生成任务</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("content文件生成失败告警", "monitor_alert")'>content文件生成失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("content文件生成数据提取", "data_extract")'>content文件生成数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("content文件生成运行分析", "data_analysis")'>content文件生成运行分析</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("content文件生成分析结果展示", "result_display")'>content文件生成分析结果展示</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  "计划时间": "2026-06-29 02:39:36",
  "任务名称": "content文件生成任务管理导入记录",
  "负责人": "刘志远",
  "实际完成": "实际完成导入值",
  "调用次数": 86,
  "优先级": "优先级导入值",
  "当前环节": "当前环节导入值",
  "接口名称": "content文件生成任务管理导入记录",
  "导入来源": "content文件生成任务管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "content文件生成任务管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "content文件生成任务管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "content文件生成任务管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "content文件生成任务管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "content文件生成任务管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "content文件生成任务管理" },

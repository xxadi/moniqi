    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增重点网络设备指令解析规则信息", "form")'>新增重点网络设备指令解析规则信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询重点网络设备指令解析规则列表", "detail")'>查询重点网络设备指令解析规则列表</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='openImport("批量导入重点网络设备指令解析规则信息")'>批量导入重点网络设备指令解析规则信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "规则类型": "重点网络设备指令解析规则管理导入记录",
  "操作模块": "操作模块导入值",
  "创建时间": "2026-06-29 02:55:41",
  "导入来源": "重点网络设备指令解析规则管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "重点网络设备指令解析规则管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "重点网络设备指令解析规则管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "重点网络设备指令解析规则管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "重点网络设备指令解析规则管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "重点网络设备指令解析规则管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "重点网络设备指令解析规则管理" },

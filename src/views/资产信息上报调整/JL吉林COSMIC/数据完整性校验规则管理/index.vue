    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增数据完整性校验规则", "form")'>新增数据完整性校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询数据完整性校验规则", "detail")'>查询数据完整性校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("数据完整性校验失败告警", "monitor_alert")'>数据完整性校验失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("数据完整性校验数据提取", "data_extract")'>数据完整性校验数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("数据完整性校验规则分析", "validate")'>数据完整性校验规则分析</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("数据完整性校验规则分析结果展示", "result_display")'>数据完整性校验规则分析结果展示</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "优先级": "优先级导入值",
  "规则编号": "IMP-776337",
  "资产类型": "数据完整性校验规则管理导入记录",
  "调用次数": 71,
  "下发时间": "2026-06-29 02:39:36",
  "执行节点": "执行节点导入值",
  "导入来源": "数据完整性校验规则管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "数据完整性校验规则管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "数据完整性校验规则管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "数据完整性校验规则管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "数据完整性校验规则管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "数据完整性校验规则管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

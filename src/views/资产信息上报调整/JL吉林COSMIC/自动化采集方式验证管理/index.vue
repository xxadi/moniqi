    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增自动化采集方式验证", "form")'>新增自动化采集方式验证</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询自动化采集方式验证", "detail")'>查询自动化采集方式验证</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("自动化采集方式验证运行效果评估", "run_evaluation")'>自动化采集方式验证运行效果评估</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "采集来源": "采集来源导入值",
  "数据源": "数据源导入值",
  "资产类型": "自动化采集方式验证管理导入记录",
  "同步时间": "2026-06-29 02:55:41",
  "优先级": "优先级导入值",
  "指令状态": "已完成",
  "导入来源": "自动化采集方式验证管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "自动化采集方式验证管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "自动化采集方式验证管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "自动化采集方式验证管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "自动化采集方式验证管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "自动化采集方式验证管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

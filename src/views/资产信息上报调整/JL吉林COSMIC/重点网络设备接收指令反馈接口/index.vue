    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-promotion" @click='handleFunction("重点网络设备接收指令反馈接口", "api")'>重点网络设备接收指令反馈接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("重点网络设备接收指令反馈数据分析", "data_analysis")'>重点网络设备接收指令反馈数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看重点网络设备接收指令反馈信息", "detail")'>查看重点网络设备接收指令反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("输入重点网络设备接收指令反馈请求任务", "input_request")'>输入重点网络设备接收指令反馈请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("输出重点网络设备接收指令反馈反馈信息", "feedback")'>输出重点网络设备接收指令反馈反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控重点网络设备接收指令反馈状态信息", "monitor")'>监控重点网络设备接收指令反馈状态信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("统计重点网络设备接收指令反馈调用信息", "statistics")'>统计重点网络设备接收指令反馈调用信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "重点网络设备接收指令反馈接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "重点网络设备接收指令反馈接口任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "重点网络设备接收指令反馈接口异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "重点网络设备接收指令反馈接口业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "重点网络设备接收指令反馈接口复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "重点网络设备接收指令反馈接口" },

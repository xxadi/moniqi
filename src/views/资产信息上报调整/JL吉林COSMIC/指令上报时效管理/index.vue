    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增指令上报时效信息", "form")'>新增指令上报时效信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询指令上报时效信息", "detail")'>查询指令上报时效信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令上报时效监控", "log")'>指令上报时效监控</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("指令上报时效告警", "audit")'>指令上报时效告警</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令上报时效查看", "operation")'>指令上报时效查看</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令上报时效分析", "data_analysis")'>指令上报时效分析</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增指令上报时效状态查询信息", "monitor")'>新增指令上报时效状态查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增指令上报时效参数设置信息", "statistics")'>新增指令上报时效参数设置信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "重试次数": 25,
  "失效时间": "2026-06-29 02:39:36",
  "处理状态": "已完成",
  "导入来源": "指令上报时效管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令上报时效管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令上报时效管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令上报时效管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令上报时效管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "指令上报时效管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

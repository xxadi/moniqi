    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-promotion" @click='handleFunction("上报结果接收接口", "api")'>上报结果接收接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("上报结果接收数据分析", "data_analysis")'>上报结果接收数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看上报结果接收信息", "detail")'>查看上报结果接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("输入上报结果接收请求任务", "input_request")'>输入上报结果接收请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("输出上报结果接收反馈信息", "feedback")'>输出上报结果接收反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控上报结果接收状态信息", "monitor")'>监控上报结果接收状态信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("统计上报结果接收调用信息", "statistics")'>统计上报结果接收调用信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "上报结果接收接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "上报结果接收接口任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "上报结果接收接口异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "上报结果接收接口业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "上报结果接收接口复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

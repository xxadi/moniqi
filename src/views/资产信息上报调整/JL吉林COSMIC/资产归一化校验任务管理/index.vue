    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产归一化校验任务", "form")'>新增资产归一化校验任务</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询资产归一化校验任务", "detail")'>查询资产归一化校验任务</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("资产归一化校验失败告警", "monitor_alert")'>资产归一化校验失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归一化校验数据提取", "data_extract")'>资产归一化校验数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归一化校验运行分析", "validate")'>资产归一化校验运行分析</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归一化校验分析结果展示", "result_display")'>资产归一化校验分析结果展示</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  "资产归属": "资产归属导入值",
  "IP地址": "IP地址导入值",
  "资产名称": "资产归一化校验任务管理导入记录",
  "纳管状态": "已完成",
  "发现方式": "发现方式导入值",
  "计划时间": "2026-06-29 02:55:41",
  "平均耗时": "平均耗时导入值",
  "工单类型": "资产归一化校验任务管理导入记录",
  "导入来源": "资产归一化校验任务管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产归一化校验任务管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产归一化校验任务管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产归一化校验任务管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产归一化校验任务管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "资产归一化校验任务管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

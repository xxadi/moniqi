    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产归属与位置校验规则", "form")'>新增资产归属与位置校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询资产归属与位置校验规则", "detail")'>查询资产归属与位置校验规则</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归属与位置校验规则运行效果评估", "run_evaluation")'>资产归属与位置校验规则运行效果评估</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归属与位置校验规则分析", "validate")'>资产归属与位置校验规则分析</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产归属与位置校验规则分析结果展示", "result_display")'>资产归属与位置校验规则分析结果展示</el-button>
        </div>
      </div>
      <cs-pagetable
...
const initialRows = [
  ...
  },
  {
    ...
    "优先级": "P1-高",
    "规则名称": "端口开放规则",
    "生效时间": "2026-05-08 14:03:52",
    "规则类型": "防护规则",
    "命中次数": 456,
    "操作时间": "2026-05-08 14:03:52",
    "接口名称": "漏洞查询接口"
  },
  {
    "ID": 8,
    "优先级": "P2-中",
    "规则名称": "资产纳管规则",
    "生效时间": "2026-05-08 15:21:37",
    "规则类型": "过滤规则",
    "命中次数": 1023,
    "操作时间": "2026-05-08 15:21:37",
    "接口名称": "指令下发接口"
  },
  {
    "ID": 9,
...
const importTemplateRow = {
  "优先级": "优先级导入值",
  "规则名称": "资产归属与位置校验规则管理导入记录",
  "生效时间": "2026-06-29 02:39:36",
  "规则类型": "资产归属与位置校验规则管理导入记录",
  "命中次数": 23,
  "操作时间": "2026-06-29 02:39:36",
  "接口名称": "资产归属与位置校验规则管理导入记录",
  "导入来源": "资产归属与位置校验规则管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产归属与位置校验规则管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产归属与位置校验规则管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产归属与位置校验规则管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产归属与位置校验规则管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "资产归属与位置校验规则管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "资产归属与位置校验规则管理" },

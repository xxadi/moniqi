    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-promotion" @click='handleFunction("指令上报接口", "api")'>指令上报接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("指令上报接口数据分析", "data_analysis")'>指令上报接口数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看指令上报接口信息", "detail")'>查看指令上报接口信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("输入指令上报接口请求任务", "input_request")'>输入指令上报接口请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("输出指令上报接口反馈信息", "feedback")'>输出指令上报接口反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控指令上报接口状态信息", "form")'>监控指令上报接口状态信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("统计指令上报接口调用信息", "monitor")'>统计指令上报接口调用信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "指令上报接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "指令上报接口任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "指令上报接口异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "指令上报接口业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "指令上报接口复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

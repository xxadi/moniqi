...
<script>
import { parseExcelFile, downloadImportTemplate, generateDefaultData } from "@/utils/index";
export default {
  name: "ImportDialog",
  props: {
    tableColumns: {
  ...
  methods: {
    ...
    async handleSubmit() {
      this.$refs.uploadForm.validate(async (valid) => {
        ...
        this.importing = true;
        try {
          // 根据tableColumns生成默认数据，所有字段都有值
          const data = generateDefaultData(this.tableColumns);
          if (!data || data.length === 0) {
            this.$message.warning("没有可导入的数据列配置");
            return;
          }

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产类别缺失信息", "form")'>新增资产类别缺失信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看资产类别缺失信息", "detail")'>查看资产类别缺失信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别缺失数据提取", "data_extract")'>资产类别缺失数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别缺失数据识别", "data_recognition")'>资产类别缺失数据识别</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别缺失分级", "grading")'>资产类别缺失分级</el-button>
          <el-button plain type="primary" icon="el-icon-time" @click='handleFunction("资产类别缺失实时计算", "realtime_compute")'>资产类别缺失实时计算</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别缺失聚合", "validate")'>资产类别缺失聚合</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("资产类别缺失可视化展示", "visualization")'>资产类别缺失可视化展示</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别缺失数据审查", "data_review")'>资产类别缺失数据审查</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "发现方式": "发现方式导入值",
  "资产名称": "资产类别缺失报表导入记录",
  "校验时间": "2026-06-29 02:39:36",
  "字段来源": "字段来源导入值",
  "扫描策略": "扫描策略导入值",
  "导入来源": "资产类别缺失报表导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产类别缺失报表导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产类别缺失报表任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产类别缺失报表异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产类别缺失报表业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "资产类别缺失报表复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-claim" @click='handleFunction("发起单IP扫描流程", "initiate_flow")'>发起单IP扫描流程</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增单IP扫描指派信息", "assign")'>新增单IP扫描指派信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增单IP扫描流程节点", "form")'>新增单IP扫描流程节点</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("单IP扫描流程节点查询", "node_query")'>单IP扫描流程节点查询</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("上传单IP扫描信息", "upload")'>上传单IP扫描信息</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("单IP扫描流程通知", "flow_notify")'>单IP扫描流程通知</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("单IP扫描结果逐级审批", "approval")'>单IP扫描结果逐级审批</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "单IP扫描流程管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "单IP扫描流程管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "单IP扫描流程管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "单IP扫描流程管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "单IP扫描流程管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产类型信息", "form")'>新增资产类型信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询资产类型列表", "detail")'>查询资产类型列表</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='openImport("批量导入资产类型信息")'>批量导入资产类型信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产类型管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产类型管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产类型管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产类型管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "资产类型管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

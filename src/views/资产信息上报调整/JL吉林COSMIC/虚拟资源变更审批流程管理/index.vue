    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-claim" @click='handleFunction("发起虚拟资源变更审批流程", "initiate_flow")'>发起虚拟资源变更审批流程</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看虚拟资源变更审批流程", "detail")'>查看虚拟资源变更审批流程</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批流程待办信息提醒", "audit")'>虚拟资源变更审批流程待办信息提醒</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批流程催办", "urge")'>虚拟资源变更审批流程催办</el-button>
          <el-button plain type="primary" icon="el-icon-message" @click='handleFunction("虚拟资源变更审批流程处理结果通知", "result_notify")'>虚拟资源变更审批流程处理结果通知</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批流程数据分析", "data_analysis")'>虚拟资源变更审批流程数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批流程数据分析结果推送", "push")'>虚拟资源变更审批流程数据分析结果推送</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增虚拟资源变更审批指派信息", "assign")'>新增虚拟资源变更审批指派信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增虚拟资源变更审批流程节点", "form")'>新增虚拟资源变更审批流程节点</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批流程节点查询", "node_query")'>虚拟资源变更审批流程节点查询</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批流程通知", "flow_notify")'>虚拟资源变更审批流程通知</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批结果逐级审批", "approval")'>虚拟资源变更审批结果逐级审批</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("虚拟资源变更审批结果拒绝和驳回", "reject")'>虚拟资源变更审批结果拒绝和驳回</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  "工单类型": "虚拟资源变更审批流程管理导入记录",
  "当前环节": "当前环节导入值",
  "关联资产": "关联资产导入值",
  "处理人": "陈思涵",
  "扫描耗时": "扫描耗时导入值",
  "数据源": "数据源导入值",
  "导入来源": "虚拟资源变更审批流程管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "虚拟资源变更审批流程管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "虚拟资源变更审批流程管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "虚拟资源变更审批流程管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "虚拟资源变更审批流程管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "虚拟资源变更审批流程管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

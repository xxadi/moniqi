    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增IP端口扫描准确性校验任务", "form")'>新增IP端口扫描准确性校验任务</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询IP端口扫描准确性校验任务", "detail")'>查询IP端口扫描准确性校验任务</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("IP端口扫描准确性校验失败告警", "monitor_alert")'>IP端口扫描准确性校验失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP端口扫描准确性校验数据提取", "data_extract")'>IP端口扫描准确性校验数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP端口扫描准确性校验运行分析", "validate")'>IP端口扫描准确性校验运行分析</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP端口扫描准确性校验分析结果展示", "result_display")'>IP端口扫描准确性校验分析结果展示</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "校验结果": "校验结果导入值",
  "数据源": "数据源导入值",
  "规则名称": "IP端口扫描准确性校验任务管理导入记录",
  "执行状态": "已完成",
  "导入来源": "IP端口扫描准确性校验任务管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "IP端口扫描准确性校验任务管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "IP端口扫描准确性校验任务管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "IP端口扫描准确性校验任务管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IP端口扫描准确性校验任务管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "IP端口扫描准确性校验任务管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "IP端口扫描准确性校验任务管理" },

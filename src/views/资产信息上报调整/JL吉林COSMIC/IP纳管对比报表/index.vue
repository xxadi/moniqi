    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增IP纳管对比信息", "form")'>新增IP纳管对比信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看IP纳管对比信息", "detail")'>查看IP纳管对比信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP纳管对比数据提取", "data_extract")'>IP纳管对比数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP纳管对比数据识别", "data_recognition")'>IP纳管对比数据识别</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP纳管对比分级", "grading")'>IP纳管对比分级</el-button>
          <el-button plain type="primary" icon="el-icon-time" @click='handleFunction("IP纳管对比实时计算", "realtime_compute")'>IP纳管对比实时计算</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP纳管对比聚合", "validate")'>IP纳管对比聚合</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("IP纳管对比可视化展示", "visualization")'>IP纳管对比可视化展示</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("IP纳管对比数据审查", "data_review")'>IP纳管对比数据审查</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "IP纳管对比报表导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "IP纳管对比报表任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "IP纳管对比报表异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "IP纳管对比报表业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "IP纳管对比报表复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

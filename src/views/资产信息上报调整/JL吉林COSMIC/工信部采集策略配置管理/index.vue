    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增工信部采集策略配置信息", "form")'>新增工信部采集策略配置信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询工信部采集策略配置", "detail")'>查询工信部采集策略配置</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  "采集来源": "采集来源导入值",
  "数据源": "数据源导入值",
  "资产类型": "工信部采集策略配置管理导入记录",
  "同步时间": "2026-06-29 02:39:36",
  "校验范围": "校验范围导入值",
  "阈值": "阈值导入值",
  "导入来源": "工信部采集策略配置管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "工信部采集策略配置管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "工信部采集策略配置管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "工信部采集策略配置管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "工信部采集策略配置管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "工信部采集策略配置管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

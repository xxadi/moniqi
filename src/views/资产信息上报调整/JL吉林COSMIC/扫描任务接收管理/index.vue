    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增扫描任务接收信息", "form")'>新增扫描任务接收信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("扫描任务接收结果监控", "monitor")'>扫描任务接收结果监控</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("扫描任务接收结果监控告警", "monitor_alert")'>扫描任务接收结果监控告警</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("扫描任务接收结果稽核", "audit")'>扫描任务接收结果稽核</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务接收数据采集", "collection")'>扫描任务接收数据采集</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务接收数据转换", "transform")'>扫描任务接收数据转换</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("扫描任务接收数据预处理", "preprocess")'>扫描任务接收数据预处理</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "扫描任务接收管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "扫描任务接收管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "扫描任务接收管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "扫描任务接收管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "扫描任务接收管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

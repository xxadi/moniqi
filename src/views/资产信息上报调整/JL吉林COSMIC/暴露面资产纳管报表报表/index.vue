    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增暴露面资产纳管报表信息", "form")'>新增暴露面资产纳管报表信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看暴露面资产纳管报表信息", "detail")'>查看暴露面资产纳管报表信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("暴露面资产纳管报表数据提取", "data_extract")'>暴露面资产纳管报表数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("暴露面资产纳管报表数据识别", "data_recognition")'>暴露面资产纳管报表数据识别</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("暴露面资产纳管报表分级", "grading")'>暴露面资产纳管报表分级</el-button>
          <el-button plain type="primary" icon="el-icon-time" @click='handleFunction("暴露面资产纳管报表实时计算", "realtime_compute")'>暴露面资产纳管报表实时计算</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("暴露面资产纳管报表聚合", "validate")'>暴露面资产纳管报表聚合</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("暴露面资产纳管报表可视化展示", "visualization")'>暴露面资产纳管报表可视化展示</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("暴露面资产纳管报表数据审查", "data_review")'>暴露面资产纳管报表数据审查</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "暴露面资产纳管报表报表导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "暴露面资产纳管报表报表任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "暴露面资产纳管报表报表异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "暴露面资产纳管报表报表业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "暴露面资产纳管报表报表复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

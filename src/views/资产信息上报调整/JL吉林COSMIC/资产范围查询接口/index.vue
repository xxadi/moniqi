    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-promotion" @click='handleFunction("资产范围查询接口", "api")'>资产范围查询接口</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产范围查询数据分析", "data_analysis")'>资产范围查询数据分析</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查看资产范围查询信息", "detail")'>查看资产范围查询信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产范围查询下发信息", "form")'>新增资产范围查询下发信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("输入资产范围查询请求任务", "input_request")'>输入资产范围查询请求任务</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("输出资产范围查询反馈信息", "feedback")'>输出资产范围查询反馈信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控资产范围查询状态信息", "monitor")'>监控资产范围查询状态信息</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("统计资产范围查询调用信息", "statistics")'>统计资产范围查询调用信息</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产范围查询接口导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产范围查询接口任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产范围查询接口异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产范围查询接口业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "资产范围查询接口复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

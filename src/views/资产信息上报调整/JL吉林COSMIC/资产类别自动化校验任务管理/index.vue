    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产类别自动化校验任务", "form")'>新增资产类别自动化校验任务</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询资产类别自动化校验任务", "detail")'>查询资产类别自动化校验任务</el-button>
          <el-button plain type="primary" icon="el-icon-warning" @click='handleFunction("资产类别自动化校验失败告警", "monitor_alert")'>资产类别自动化校验失败告警</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别自动化校验数据提取", "data_extract")'>资产类别自动化校验数据提取</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别自动化校验运行分析", "validate")'>资产类别自动化校验运行分析</el-button>
          <el-button plain type="primary" icon="el-icon-data-analysis" @click='handleFunction("资产类别自动化校验分析结果展示", "result_display")'>资产类别自动化校验分析结果展示</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "发现方式": "发现方式导入值",
  "操作人": "王建国",
  "规则名称": "资产类别自动化校验任务管理导入记录",
  "工单状态": "已完成",
  "导入来源": "资产类别自动化校验任务管理导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "资产类别自动化校验任务管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "资产类别自动化校验任务管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "资产类别自动化校验任务管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "资产类别自动化校验任务管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "资产类别自动化校验任务管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "资产类别自动化校验任务管理" },

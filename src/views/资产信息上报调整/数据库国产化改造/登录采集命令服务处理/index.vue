    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("系统业务监测配置登录采集命令服务处理信息修改", "configure")'>系统业务监测配置登录采集命令服务处理信息修改</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("系统业务监测配置信息写入登录采集命令服务", "configure")'>系统业务监测配置信息写入登录采集命令服务</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询网关设备信息是否在登录采集命令服务", "query")'>查询网关设备信息是否在登录采集命令服务</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("存在则写入网关设备信息更新，不存在则插入网关设备信息数据至登录采集命令服务", "operation")'>存在则写入网关设备信息更新，不存在则插入网关设备信息数据至登录采集命令服务</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "登录采集命令服务处理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "登录采集命令服务处理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "登录采集命令服务处理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "登录采集命令服务处理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "登录采集命令服务处理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("接收Agent代理任务调度模块线程处理管理改造请求", "receive_request")'>接收Agent代理任务调度模块线程处理管理改造请求</el-button>
          <el-button plain type="primary" icon="el-icon-s-promotion" @click='handleFunction("向Agent代理任务调度模块发送负载均衡调用管理改造请求", "send")'>向Agent代理任务调度模块发送负载均衡调用管理改造请求</el-button>
          <el-button plain type="primary" icon="el-icon-document-checked" @click='handleFunction("接收Agent代理任务调度模块反馈的线程管理改造信息", "receive_feedback")'>接收Agent代理任务调度模块反馈的线程管理改造信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-claim" @click='handleFunction("调用Agent代理任务调度模块线程处理对应管理改造流程", "invoke")'>调用Agent代理任务调度模块线程处理对应管理改造流程</el-button>
          <el-button plain type="primary" icon="el-icon-finished" @click='handleFunction("反馈Agent代理任务调度模块线程处理管理改造结果", "feedback_result")'>反馈Agent代理任务调度模块线程处理管理改造结果</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "耗时(ms)": "",
  "更新时间": "",
  "导入来源": "Agent代理任务调度模块管理改造导入模板.xlsx"
};
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "Agent代理任务调度模块管理改造导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "Agent代理任务调度模块管理改造任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "Agent代理任务调度模块管理改造异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "Agent代理任务调度模块管理改造业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "Agent代理任务调度模块管理改造复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "Agent代理任务调度模块管理改造" },

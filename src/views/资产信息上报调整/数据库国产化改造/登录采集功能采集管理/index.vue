    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("录入调度登录采集功能采集配置", "configure")'>录入调度登录采集功能采集配置</el-button>
          <el-button plain type="primary" icon="el-icon-s-opportunity" @click='handleFunction("调度唤醒登录采集功能采集进程", "dispatch")'>调度唤醒登录采集功能采集进程</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("读取进行登录采集功能采集执行状态", "query")'>读取进行登录采集功能采集执行状态</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("输出登录采集功能采集任务执行结果", "feedback")'>输出登录采集功能采集任务执行结果</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "登录采集功能采集管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "登录采集功能采集管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "登录采集功能采集管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "登录采集功能采集管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "登录采集功能采集管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

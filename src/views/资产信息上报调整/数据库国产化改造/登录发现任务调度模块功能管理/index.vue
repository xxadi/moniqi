    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("接收登录发现任务调度模块线程处理功能管理请求", "receive_request")'>接收登录发现任务调度模块线程处理功能管理请求</el-button>
          <el-button plain type="primary" icon="el-icon-s-promotion" @click='handleFunction("向登录发现任务调度模块发送负载均衡调用功能管理请求", "send")'>向登录发现任务调度模块发送负载均衡调用功能管理请求</el-button>
          <el-button plain type="primary" icon="el-icon-document-checked" @click='handleFunction("接收登录发现任务调度模块反馈的线程功能管理信息", "receive_feedback")'>接收登录发现任务调度模块反馈的线程功能管理信息</el-button>
          <el-button plain type="primary" icon="el-icon-s-claim" @click='handleFunction("调用登录发现任务调度模块线程处理对应功能管理流程", "invoke")'>调用登录发现任务调度模块线程处理对应功能管理流程</el-button>
          <el-button plain type="primary" icon="el-icon-finished" @click='handleFunction("反馈登录发现任务调度模块线程处理功能管理结果", "feedback_result")'>反馈登录发现任务调度模块线程处理功能管理结果</el-button>
        </div>
      </div>
      <cs-pagetable
...
export default {
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "登录发现任务调度模块功能管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "登录发现任务调度模块功能管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "登录发现任务调度模块功能管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "登录发现任务调度模块功能管理业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "登录发现任务调度模块功能管理复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
    ...
    buildBusinessDetail(functionName) {
      ...
      const fields = [
        ...
        { label: "所属页面", value: "登录发现任务调度模块功能管理" },

    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询虚拟资源的当前状态", "detail")'>查询虚拟资源的当前状态</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("导出虚拟资源查询的操作结果", "export")'>导出虚拟资源查询的操作结果</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("保存虚拟资源查询记录", "detail")'>保存虚拟资源查询记录</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询虚拟资源查询历史", "detail")'>查询虚拟资源查询历史</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加虚拟资源状态查询规则", "add")'>添加虚拟资源状态查询规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "上报时间": "2026-05-08 09:35:44",
  "ID": "",
  "导入来源": "虚拟资源状态查询导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage318",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "虚拟资源状态查询导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "虚拟资源状态查询任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "虚拟资源状态查询异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "虚拟资源状态查询状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "虚拟资源状态查询回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "虚拟资源状态查询业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "虚拟资源状态查询复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

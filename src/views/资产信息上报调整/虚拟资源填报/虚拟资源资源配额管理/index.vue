    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("管理虚拟资源的使用配额", "operation")'>管理虚拟资源的使用配额</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("导出虚拟资源配额管理结果", "export")'>导出虚拟资源配额管理结果</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("保存虚拟资源配额管理记录", "operation")'>保存虚拟资源配额管理记录</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询虚拟资源配额管理历史", "detail")'>查询虚拟资源配额管理历史</el-button>
          <el-button plain type="primary" icon="el-icon-plus" @click='handleFunction("添加虚拟资源配额管理规则", "add")'>添加虚拟资源配额管理规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "上报时间": "2026-05-08 15:21:37",
  "ID": "",
  "导入来源": "虚拟资源资源配额管理导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage323",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "虚拟资源资源配额管理导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "虚拟资源资源配额管理任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "虚拟资源资源配额管理异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "虚拟资源资源配额管理状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "虚拟资源资源配额管理回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "虚拟资源资源配额管理业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "虚拟资源资源配额管理复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

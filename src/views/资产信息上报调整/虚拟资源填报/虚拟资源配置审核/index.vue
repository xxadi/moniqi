    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("审核虚拟资源的配置变更信息", "edit")'>审核虚拟资源的配置变更信息</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("导出虚拟资源配置审核操作结果", "export")'>导出虚拟资源配置审核操作结果</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("添加虚拟资源配置审核标准与规则", "edit")'>添加虚拟资源配置审核标准与规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("审核虚拟资源配置的审批请求", "edit")'>审核虚拟资源配置的审批请求</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载虚拟资源配置审核结果", "export")'>下载虚拟资源配置审核结果</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("保存虚拟资源配置审核记录", "edit")'>保存虚拟资源配置审核记录</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("查询虚拟资源配置审核历史", "edit")'>查询虚拟资源配置审核历史</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("添加虚拟资源配置审核规则", "edit")'>添加虚拟资源配置审核规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "上报时间": "2026-05-08 16:08:25",
  "ID": "",
  "导入来源": "虚拟资源配置审核导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage314",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "虚拟资源配置审核导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "虚拟资源配置审核任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "虚拟资源配置审核异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "虚拟资源配置审核状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "虚拟资源配置审核回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "虚拟资源配置审核业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "虚拟资源配置审核复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

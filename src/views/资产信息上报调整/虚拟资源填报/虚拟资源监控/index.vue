    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("监控虚拟资源的健康状态和使用情况", "detail")'>监控虚拟资源的健康状态和使用情况</el-button>
          <el-button plain type="primary" icon="el-icon-download" @click='handleFunction("下载虚拟资源健康监控报告", "export")'>下载虚拟资源健康监控报告</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("保存虚拟资源监控数据", "detail")'>保存虚拟资源监控数据</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询虚拟资源监控历史记录", "detail")'>查询虚拟资源监控历史记录</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("添加虚拟资源监控配置规则", "edit")'>添加虚拟资源监控配置规则</el-button>
        </div>
      </div>
      <cs-pagetable
...
const importTemplateRow = {
  ...
  "ID": "",
  "导入来源": "虚拟资源监控导入模板.xlsx"
};

export default {
  name: "GeneratedFeaturePage328",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    exportRows(functionName) {
      ...
      exportXLSX(rows, functionName || "虚拟资源监控导出");
    ...
    buildExportRows(functionName) {
      ...
      if (/执行状态|日志/.test(functionName)) {
        return [
          ...
          { 名称: "虚拟资源监控任务调度", 状态: "执行中", 日志: "正在等待节点返回执行结果" },
          { 名称: "虚拟资源监控异常检查", 状态: "已告警", 日志: "发现1条超时记录，已进入重试队列" },
        ];
      }
      ...
      if (/接口|调用/.test(functionName)) {
        return [
          ...
          { 接口名称: "虚拟资源监控状态同步接口", 请求方式: "GET", 调用状态: "200 OK", 平均耗时: "93ms" },
          { 接口名称: "虚拟资源监控回执接口", 请求方式: "POST", 调用状态: "202 ACCEPTED", 平均耗时: "141ms" },
        ];
      }
      ...
      return [
        ...
        { 业务名称: "虚拟资源监控业务记录", 业务状态: "已处理", 处理时间: "2026-05-08 10:05:00", 说明: "模拟业务记录" },
        { 业务名称: "虚拟资源监控复核记录", 业务状态: "待复核", 处理时间: "2026-05-08 10:10:00", 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";

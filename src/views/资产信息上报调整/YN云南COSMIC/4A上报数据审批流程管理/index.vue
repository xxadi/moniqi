    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-claim" @click='handleFunction("发起4A上报数据审批流程", "workflow")'>发起4A上报数据审批流程</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增4A上报数据审批指派信息", "form")'>新增4A上报数据审批指派信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增4A上报数据审批流程", "form")'>新增4A上报数据审批流程</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改4A上报数据审批流程信息", "config")'>修改4A上报数据审批流程信息</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("4A上报数据审批流程删除", "operation")'>4A上报数据审批流程删除</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("4A上报数据审批流程查询", "detail")'>4A上报数据审批流程查询</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("4A上报数据审批附件上传", "form")'>4A上报数据审批附件上传</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("4A上报数据审批流程图查看", "preview")'>4A上报数据审批流程图查看</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("4A上报数据审批流程通知", "audit")'>4A上报数据审批流程通知</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("4A上报数据审批结果逐级审批", "audit")'>4A上报数据审批结果逐级审批</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("4A上报数据审批结果拒绝和驳回", "approval")'>4A上报数据审批结果拒绝和驳回</el-button>
        </div>
      </div>
      <cs-pagetable
...
const initialRows = [
  {
    ...
    "流程环节ID": "B-12429",
    "4A上报数据审批编号": "B-12429",
    "4A上报数据审批名称": "4A上报数据审批流程管理-1",
    "发起4A上报数据审批状态": "已取消",
    "发起审批时间": "2026-05-09 09:47:56",
    "发起5A上报数据审批状态": "已取消",
    "发起6A上报数据审批状态": "已取消",
    "发起7A上报数据审批状态": "已取消",
    "发起8A上报数据审批状态": "已取消",
  ...
  },
  {
    ...
    "流程环节ID": "B-20348",
    "4A上报数据审批编号": "B-20348",
    "4A上报数据审批名称": "4A上报数据审批流程管理-2",
    "发起4A上报数据审批状态": "已驳回",
    "发起审批时间": "2026-05-08 09:12:18",
    "发起5A上报数据审批状态": "已驳回",
    "发起6A上报数据审批状态": "已驳回",
    "发起7A上报数据审批状态": "已驳回",
    "发起8A上报数据审批状态": "已驳回",
  ...
  },
  {
    ...
    "流程环节ID": "B-28267",
    "4A上报数据审批编号": "B-28267",
    "4A上报数据审批名称": "4A上报数据审批流程管理-3",
    "发起4A上报数据审批状态": "待审核",
    "发起审批时间": "2026-05-08 09:35:44",
    "发起5A上报数据审批状态": "待审核",
    "发起6A上报数据审批状态": "待审核",
    "发起7A上报数据审批状态": "待审核",
    "发起8A上报数据审批状态": "待审核",
  ...
  },
  {
    ...
    "流程环节ID": "B-36186",
    "4A上报数据审批编号": "B-36186",
    "4A上报数据审批名称": "4A上报数据审批流程管理-4",
    "发起4A上报数据审批状态": "已处理",
    "发起审批时间": "2026-05-08 10:08:06",
    "发起5A上报数据审批状态": "已处理",
    "发起6A上报数据审批状态": "已处理",
    "发起7A上报数据审批状态": "已处理",
    "发起8A上报数据审批状态": "已处理",
  ...
  },
  {
    ...
    "流程环节ID": "B-44105",
    "4A上报数据审批编号": "B-44105",
    "4A上报数据审批名称": "4A上报数据审批流程管理-5",
    "发起4A上报数据审批状态": "处理中",
    "发起审批时间": "2026-05-08 10:42:31",
    "发起5A上报数据审批状态": "处理中",
    "发起6A上报数据审批状态": "处理中",
    "发起7A上报数据审批状态": "处理中",
    "发起8A上报数据审批状态": "处理中",
  ...
  },
  {
    ...
    "流程环节ID": "B-52024",
    "4A上报数据审批编号": "B-52024",
    "4A上报数据审批名称": "4A上报数据审批流程管理-6",
    "发起4A上报数据审批状态": "待处理",
    "发起审批时间": "2026-05-08 11:16:09",
    "发起5A上报数据审批状态": "待处理",
    "发起6A上报数据审批状态": "待处理",
    "发起7A上报数据审批状态": "待处理",
    "发起8A上报数据审批状态": "待处理",
  ...
  },
  {
    ...
    "流程环节ID": "B-59943",
    "4A上报数据审批编号": "B-59943",
    "4A上报数据审批名称": "4A上报数据审批流程管理-7",
    "发起4A上报数据审批状态": "处理失败",
    "发起审批时间": "2026-05-08 14:03:52",
    "发起5A上报数据审批状态": "处理失败",
    "发起6A上报数据审批状态": "处理失败",
    "发起7A上报数据审批状态": "处理失败",
    "发起8A上报数据审批状态": "处理失败",
  ...
  },
  {
    ...
    "流程环节ID": "B-67862",
    "4A上报数据审批编号": "B-67862",
    "4A上报数据审批名称": "4A上报数据审批流程管理-8",
    "发起4A上报数据审批状态": "已暂停",
    "发起审批时间": "2026-05-08 15:21:37",
    "发起5A上报数据审批状态": "已暂停",
    "发起6A上报数据审批状态": "已暂停",
    "发起7A上报数据审批状态": "已暂停",
    "发起8A上报数据审批状态": "已暂停",
  ...
  },
  {
    ...
    "流程环节ID": "B-75781",
    "4A上报数据审批编号": "B-75781",
    "4A上报数据审批名称": "4A上报数据审批流程管理-9",
    "发起4A上报数据审批状态": "已完成",
    "发起审批时间": "2026-05-08 16:08:25",
    "发起5A上报数据审批状态": "已完成",
    "发起6A上报数据审批状态": "已完成",
    "发起7A上报数据审批状态": "已完成",
    "发起8A上报数据审批状态": "已完成",
  ...
  },
  {
    ...
    "流程环节ID": "B-83700",
    "4A上报数据审批编号": "B-83700",
    "4A上报数据审批名称": "4A上报数据审批流程管理-10",
    "发起4A上报数据审批状态": "排队中",
    "发起审批时间": "2026-05-09 08:33:11",
    "发起5A上报数据审批状态": "排队中",
    "发起6A上报数据审批状态": "排队中",
    "发起7A上报数据审批状态": "排队中",
    "发起8A上报数据审批状态": "排队中",

  ...
  <div>
    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增4A与资产数据差异监控规则", "form")'>新增4A与资产数据差异监控规则</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改4A与资产数据差异监控规则", "form")'>修改4A与资产数据差异监控规则</el-button>
          <el-button plain type="danger" icon="el-icon-delete" @click='handleFunction("删除4A与资产数据差异监控规则", "delete")'>删除4A与资产数据差异监控规则</el-button>
          <el-button plain type="primary" icon="el-icon-s-tools" @click='handleFunction("4A与资产数据差异监控规则配置", "config")'>4A与资产数据差异监控规则配置</el-button>
          <el-button plain type="danger" icon="el-icon-delete" @click='handleFunction("4A资产数据差异错误事件通知删除", "delete")'>4A资产数据差异错误事件通知删除</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("4A资产数据差异错误事件通知查询", "detail")'>4A资产数据差异错误事件通知查询</el-button>
        </div>
    ...
    <el-dialog :title="recvFbTitle" :visible.sync="recvFbVisible" width="660px" append-to-body>
      ...
      <span slot="footer">
        ...
        <el-button v-if="activeFunction === '4A资产数据差异错误事件通知删除'" type="primary" @click="confirmDeleteNotification">确认删除</el-button>
...
const rows = [
  { ID: 1, 规则编号: "JK-001", 规则名称: "4A资产差异数量监控规则", 监控指标: "差异数量", 阈值: ">100", 监控频率: "每小时", 启用状态: "启用" },
  { ID: 2, 规则编号: "JK-002", 规则名称: "4A资产差异比例监控规则", 监控指标: "差异比例", 阈值: ">5%", 监控频率: "每小时", 启用状态: "启用" },
  { ID: 3, 规则编号: "JK-003", 规则名称: "4A资产新增资产监控规则", 监控指标: "新增数量", 阈值: ">50", 监控频率: "每日", 启用状态: "启用" },
  { ID: 4, 规则编号: "JK-004", 规则名称: "4A资产移除资产监控规则", 监控指标: "移除数量", 阈值: ">20", 监控频率: "每日", 启用状态: "停用" },
  { ID: 5, 规则编号: "JK-005", 规则名称: "4A资产变更频次监控规则", 监控指标: "变更次数", 阈值: ">10", 监控频率: "每小时", 启用状态: "启用" },
];
const cols = [
  { prop: "规则编号", label: "规则编号", type: "text", search: true },
  { prop: "规则名称", label: "规则名称", type: "text", search: true },
  { prop: "监控指标", label: "监控指标", type: "text", search: true },
  { prop: "阈值", label: "阈值", type: "text" },
  { prop: "监控频率", label: "监控频率", type: "select", options: [{ value: "每小时", label: "每小时" }, { value: "每日", label: "每日" }, { value: "实时", label: "实时" }], search: true },
  { prop: "启用状态", label: "启用状态", type: "select", options: [{ value: "启用", label: "启用" }, { value: "停用", label: "停用" }], search: true },
  { slot: "operate", label: "操作" },
];
const defaultRow = { 规则编号: "", 规则名称: "", 监控指标: "", 阈值: "", 监控频率: "每小时", 启用状态: "启用" };
...
export default {
  name: "4AYN_4AYuZiChanShuJuChaYiJianKongGuiZeGuanLi",
  data() {
    return {
  ...
  methods: {
    ...
    add() { this.formMode = "add"; this.formTitle = "新增监控规则"; this.formData = { ...defaultRow }; this.formVisible = true; },
    edit(row) { this.formMode = "edit"; this.formTitle = "修改监控规则"; this.formData = { ...row }; this.formVisible = true; },
    detail(row) { this.formMode = "detail"; this.formTitle = "监控规则详情"; this.formData = { ...row }; this.formVisible = true; },
    submitForm() {
    ...
    confirmDeleteNotification() {
      this.$confirm("确定要删除错误事件通知记录吗？", "删除确认", {
        ...
        this.fetchData();
        this.$message.success("错误事件通知删除成功");
      }).catch(() => {});
    },
    handleFunction(functionName, extraType) {
      ...
      if (extraType === "delete") {
        ...
        if (functionName === '4A资产数据差异错误事件通知删除') {
          this.recvFbTitle = functionName;
          this.recvFbFields = [
            { label: "删除范围", value: "监控规则记录" },
            { label: "通知类型", value: "错误事件通知" },
            { label: "影响范围", value: "数据差异监控" },
            { label: "删除时间", value: this.now() },
            { label: "操作状态", value: "待确认" },
          ];
          this.recvFbPreview = this.allTableData.slice(0, 3).map((r, i) => ({
            字段: "记录" + (i + 1),
            值: r.监控名称 || r.规则名称 || "监控规则-" + (i + 1),
            状态: "正常"
          }));

  ...
  <div>
    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("定义4A数据录入规则库信息", "define_entry")'>定义4A数据录入规则库信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改4A数据录入规则库信息", "modify_entry")'>修改4A数据录入规则库信息</el-button>
          <el-button plain type="danger" icon="el-icon-delete" @click='handleFunction("删除4A数据录入规则库信息", "delete_entry")'>删除4A数据录入规则库信息</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("4A数据录入规则库数据查询", "query_entry")'>4A数据录入规则库数据查询</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("4A数据录入规则库数据上传", "import_entry")'>4A数据录入规则库数据上传</el-button>
          <el-button plain type="primary" icon="el-icon-monitor" @click='handleFunction("4A数据录入规则库数据接口监控", "monitor_entry")'>4A数据录入规则库数据接口监控</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询4A数据录入规则库汇总报告详情", "query_summary")'>查询4A数据录入规则库汇总报告详情</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("查询4A数据录入规则库明细报告详情", "query_detail")'>查询4A数据录入规则库明细报告详情</el-button>
        </div>
    ...
    <el-dialog :title="queryDynTitle" :visible.sync="queryDynVisible" width="740px" append-to-body>
      <template v-if="queryDynMode === 'table'">
        <el-alert title="录入规则库数据查询结果" type="info" :closable="false" show-icon style="margin-bottom:16px;"></el-alert>
        <el-table :data="queryDynData" border size="mini" style="width:100%" max-height="400">
          ...
          <el-table-column prop="录入类型" label="录入类型" width="100"></el-table-column>
          <el-table-column prop="数据来源" label="数据来源" width="100"></el-table-column>
          ...
          <el-table-column prop="创建时间" label="创建时间" width="160"></el-table-column>
        </el-table>
      </template>
...
const rows = [
  { ID: 1, 规则编号: "LR-001", 规则名称: "4A账号录入规则", 录入类型: "自动录入", 数据来源: "4A平台", 规则状态: "启用", 创建时间: "2026-05-18 10:00:00", 更新时间: "2026-06-01 09:30:00" },
  { ID: 2, 规则编号: "LR-002", 规则名称: "4A资源录入规则", 录入类型: "半自动录入", 数据来源: "CMDB", 规则状态: "启用", 创建时间: "2026-05-18 11:00:00", 更新时间: "2026-06-02 14:20:00" },
  { ID: 3, 规则编号: "LR-003", 规则名称: "4A权限录入规则", 录入类型: "自动录入", 数据来源: "4A平台", 规则状态: "停用", 创建时间: "2026-05-19 08:30:00", 更新时间: "2026-06-03 10:15:00" },
  { ID: 4, 规则编号: "LR-004", 规则名称: "4A组织录入规则", 录入类型: "手动录入", 数据来源: "HR系统", 规则状态: "启用", 创建时间: "2026-05-20 14:00:00", 更新时间: "2026-06-04 16:40:00" },
  { ID: 5, 规则编号: "LR-005", 规则名称: "4A角色录入规则", 录入类型: "自动录入", 数据来源: "4A平台", 规则状态: "启用", 创建时间: "2026-05-21 09:00:00", 更新时间: "2026-06-05 08:50:00" },
];
const cols = [
  { prop: "规则编号", label: "规则编号", type: "text", search: true },
  { prop: "规则名称", label: "规则名称", type: "text", search: true },
  { prop: "录入类型", label: "录入类型", type: "select", options: [{ value: "自动录入", label: "自动录入" }, { value: "半自动录入", label: "半自动录入" }, { value: "手动录入", label: "手动录入" }], search: true },
  { prop: "数据来源", label: "数据来源", type: "text" },
...
const defaultRow = { 规则编号: "", 规则名称: "", 录入类型: "", 数据来源: "", 规则状态: "启用", 创建时间: "", 更新时间: "" };
...
export default {
  name: "4AYN_4AShuJuLuRuGuiZeKuGuanLi",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
      ...
      if (extraType) {
        ...
        if (extraType === 'query_detail') {
          ...
          this.queryDynData = this.allTableData.map((r, i) => ({
            '规则名称': r.规则名称,
            '录入类型': r.录入类型,
            '执行次数': [85, 168, 42, 210, 96][i % 5],
            '通过率': ['97%', '99%', '92%', '96%', '98%'][i % 5],
            '平均耗时': ['35ms', '95ms', '28ms', '180ms', '55ms'][i % 5],
            '最近执行': '2026-06-0' + (7 - i) + ' 11:00:00',
    ...
    add() {
      this.formMode = "add";
      this.formTitle = "新增录入规则";
      this.formData = { ...defaultRow };
      this.formVisible = true;
    },
    edit(row) {
      this.formMode = "edit";
      this.formTitle = "修改录入规则";
      this.formData = { ...row };
      this.formVisible = true;
    },
    detail(row) {
    ...
    exportFile() {
      exportXLSX(this.allTableData, "4A数据录入规则库");
    },

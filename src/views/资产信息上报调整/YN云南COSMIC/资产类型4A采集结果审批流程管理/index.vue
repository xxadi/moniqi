    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-claim" @click='handleFunction("发起资产类型4A采集结果审批流程", "workflow")'>发起资产类型4A采集结果审批流程</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产类型4A采集结果审批指派信息", "form")'>新增资产类型4A采集结果审批指派信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增资产类型4A采集结果审批流程节点", "form")'>新增资产类型4A采集结果审批流程节点</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改资产类型4A采集结果审批流程节点信息", "config")'>修改资产类型4A采集结果审批流程节点信息</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("资产类型4A采集结果审批流程节点删除", "operation")'>资产类型4A采集结果审批流程节点删除</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("资产类型4A采集结果审批流程节点查询", "detail")'>资产类型4A采集结果审批流程节点查询</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("资产类型4A采集结果审批附件上传", "form")'>资产类型4A采集结果审批附件上传</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("资产类型4A采集结果审批流程图查看", "preview")'>资产类型4A采集结果审批流程图查看</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("资产类型4A采集结果审批流程通知", "transform")'>资产类型4A采集结果审批流程通知</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("资产类型4A采集结果审批结果逐级审批", "transform")'>资产类型4A采集结果审批结果逐级审批</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("资产类型4A采集结果审批结果拒绝和驳回", "approval")'>资产类型4A采集结果审批结果拒绝和驳回</el-button>
        </div>
      </div>
      <cs-pagetable
...
const initialRows = [
  {
    "ID": 1,
    "资产类型4A采集结果审批ID": "X-56911",
    "资产类型4A采集结果审批流程环节ID": "X-56911",
    "资产类型4A采集结果审批编号": "X-56911",
    "资产类型4A采集结果审批名称": "待审核",
    "发起资产类型4A采集结果审批状态": "待审核",
    "发起资产类型4A采集结果审批时间": "2026-05-08 09:35:44",
    "资产类型4A采集结果审批属性": "待审核",
    "资产类型4A采集结果审批发起时间": "2026-05-08 09:35:44",
    "资产类型4A采集结果审批流程id": "待审核",
    "资产类型4A采集结果审批流程环节": "待审核",
    "资产类型4A采集结果审批创建时间": "2026-05-08 09:35:44"
  },
  {
    "ID": 2,
    "资产类型4A采集结果审批ID": "X-64830",
    "资产类型4A采集结果审批流程环节ID": "X-64830",
    "资产类型4A采集结果审批编号": "X-64830",
    "资产类型4A采集结果审批名称": "已处理",
    "发起资产类型4A采集结果审批状态": "已处理",
    "发起资产类型4A采集结果审批时间": "2026-05-08 10:08:06",
    "资产类型4A采集结果审批属性": "已处理",
    "资产类型4A采集结果审批发起时间": "2026-05-08 10:08:06",
    "资产类型4A采集结果审批流程id": "已处理",
    "资产类型4A采集结果审批流程环节": "已处理",
    "资产类型4A采集结果审批创建时间": "2026-05-08 10:08:06"
  },
  {
    "ID": 3,
    "资产类型4A采集结果审批ID": "X-72749",
    "资产类型4A采集结果审批流程环节ID": "X-72749",
    "资产类型4A采集结果审批编号": "X-72749",
    "资产类型4A采集结果审批名称": "处理中",
    "发起资产类型4A采集结果审批状态": "处理中",
    "发起资产类型4A采集结果审批时间": "2026-05-08 10:42:31",
    "资产类型4A采集结果审批属性": "处理中",
    "资产类型4A采集结果审批发起时间": "2026-05-08 10:42:31",
    "资产类型4A采集结果审批流程id": "处理中",
    "资产类型4A采集结果审批流程环节": "处理中",
    "资产类型4A采集结果审批创建时间": "2026-05-08 10:42:31"
  },
  {
    "ID": 4,
    "资产类型4A采集结果审批ID": "X-80668",
    "资产类型4A采集结果审批流程环节ID": "X-80668",
    "资产类型4A采集结果审批编号": "X-80668",
    "资产类型4A采集结果审批名称": "待处理",
    "发起资产类型4A采集结果审批状态": "待处理",
    "发起资产类型4A采集结果审批时间": "2026-05-08 11:16:09",
    "资产类型4A采集结果审批属性": "待处理",
    "资产类型4A采集结果审批发起时间": "2026-05-08 11:16:09",
    "资产类型4A采集结果审批流程id": "待处理",
    "资产类型4A采集结果审批流程环节": "待处理",
    "资产类型4A采集结果审批创建时间": "2026-05-08 11:16:09"
  },
  {
    "ID": 5,
    "资产类型4A采集结果审批ID": "X-88587",
    "资产类型4A采集结果审批流程环节ID": "X-88587",
    "资产类型4A采集结果审批编号": "X-88587",
    "资产类型4A采集结果审批名称": "处理失败",
    "发起资产类型4A采集结果审批状态": "处理失败",
    "发起资产类型4A采集结果审批时间": "2026-05-08 14:03:52",
    "资产类型4A采集结果审批属性": "处理失败",
    "资产类型4A采集结果审批发起时间": "2026-05-08 14:03:52",
    "资产类型4A采集结果审批流程id": "处理失败",
    "资产类型4A采集结果审批流程环节": "处理失败",
    "资产类型4A采集结果审批创建时间": "2026-05-08 14:03:52"
  },
  {
    "ID": 6,
    "资产类型4A采集结果审批ID": "X-96506",
    "资产类型4A采集结果审批流程环节ID": "X-96506",
    "资产类型4A采集结果审批编号": "X-96506",
    "资产类型4A采集结果审批名称": "已暂停",
    "发起资产类型4A采集结果审批状态": "已暂停",
    "发起资产类型4A采集结果审批时间": "2026-05-08 15:21:37",
    "资产类型4A采集结果审批属性": "已暂停",
    "资产类型4A采集结果审批发起时间": "2026-05-08 15:21:37",
    "资产类型4A采集结果审批流程id": "已暂停",
    "资产类型4A采集结果审批流程环节": "已暂停",
    "资产类型4A采集结果审批创建时间": "2026-05-08 15:21:37"
  },
  {
    "ID": 7,
    "资产类型4A采集结果审批ID": "X-104425",
    "资产类型4A采集结果审批流程环节ID": "X-104425",
    "资产类型4A采集结果审批编号": "X-104425",
    "资产类型4A采集结果审批名称": "已完成",
    "发起资产类型4A采集结果审批状态": "已完成",
    "发起资产类型4A采集结果审批时间": "2026-05-08 16:08:25",
    "资产类型4A采集结果审批属性": "已完成",
    "资产类型4A采集结果审批发起时间": "2026-05-08 16:08:25",
    "资产类型4A采集结果审批流程id": "已完成",
    "资产类型4A采集结果审批流程环节": "已完成",
    "资产类型4A采集结果审批创建时间": "2026-05-08 16:08:25"
  }
];
const tableColumns = [
  {
  ...
  {
    "prop": "资产类型4A采集结果审批ID",
    "label": "资产类型4A采集结果审批ID",
    "type": "text",
    "search": true,
    "showTooltip": true
  },
  {
    "prop": "资产类型4A采集结果审批流程环节ID",
    "label": "资产类型4A采集结果审批流程环节ID",
    "type": "text",
    "search": true,
    "showTooltip": true
  },
  ...
  {
    "prop": "资产类型4A采集结果审批名称",
    "label": "资产类型4A采集结果审批名称",
    "type": "text",
    "search": true,
    "showTooltip": true
  },
  {
    "prop": "发起资产类型4A采集结果审批状态",
    "label": "发起资产类型4A采集结果审批状态",
    "type": "text",
    "search": true,
    "showTooltip": true
  },
  {
    "prop": "发起资产类型4A采集结果审批时间",
    "label": "发起资产类型4A采集结果审批时间",
    "type": "text",
    "search": false,
    "showTooltip": true,
    "width": 170
  ...
  {
    "prop": "资产类型4A采集结果审批属性",
    "label": "资产类型4A采集结果审批属性",
    "type": "text",
    "search": true,
    "showTooltip": true
  },
  {
    "prop": "资产类型4A采集结果审批发起时间",
    "label": "资产类型4A采集结果审批发起时间",
    "type": "text",
    "search": false,
    "showTooltip": true,
    "width": 170
  ...
  {
    "prop": "资产类型4A采集结果审批流程id",
    "label": "资产类型4A采集结果审批流程id",
    "type": "text",
    "search": true,
    "showTooltip": true
  },
  {
    "prop": "资产类型4A采集结果审批流程环节",
    "label": "资产类型4A采集结果审批流程环节",
    "type": "text",
    "search": true,
    "showTooltip": true
  },
  {
    "prop": "资产类型4A采集结果审批创建时间",
    "label": "资产类型4A采集结果审批创建时间",
    "type": "text",
    "search": false,
    "showTooltip": true,
    "width": 170
...
const importTemplateRow = {
  ...
  "资产类型4A采集结果审批流程环节": "",
  "资产类型4A采集结果审批创建时间": "",
  "导入来源": "资产类型4A采集结果审批流程管理导入模板.xlsx"
};

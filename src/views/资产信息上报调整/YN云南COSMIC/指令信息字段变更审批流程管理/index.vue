<template>
  <div>
    ...
    <div class="mt10 crud-container">
      <div class="mb10 operate-container">
        <div class="function-actions">
          <el-button plain type="primary" icon="el-icon-s-claim" @click='handleFunction("发起指令信息字段变更审批流程")'>发起指令信息字段变更审批流程</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增指令信息字段变更审批指派信息")'>新增指令信息字段变更审批指派信息</el-button>
          <el-button plain type="primary" icon="el-icon-edit-outline" @click='handleFunction("新增指令信息字段变更审批流程")'>新增指令信息字段变更审批流程</el-button>
          <el-button plain type="primary" icon="el-icon-edit" @click='handleFunction("修改指令信息字段变更审批流程信息")'>修改指令信息字段变更审批流程信息</el-button>
          <el-button plain type="danger" icon="el-icon-delete" @click='handleFunction("指令信息字段变更审批流程删除")'>指令信息字段变更审批流程删除</el-button>
          <el-button plain type="primary" icon="el-icon-view" @click='handleFunction("指令信息字段变更审批流程查询")'>指令信息字段变更审批流程查询</el-button>
          <el-button plain type="primary" icon="el-icon-upload2" @click='handleFunction("指令信息字段变更审批附件上传")'>指令信息字段变更审批附件上传</el-button>
          <el-button plain type="primary" icon="el-icon-s-operation" @click='handleFunction("指令信息字段变更审批流程图查看")'>指令信息字段变更审批流程图查看</el-button>
          <el-button plain type="primary" icon="el-icon-bell" @click='handleFunction("指令信息字段变更审批流程通知")'>指令信息字段变更审批流程通知</el-button>
          <el-button plain type="primary" icon="el-icon-s-check" @click='handleFunction("指令信息字段变更审批结果逐级审批")'>指令信息字段变更审批结果逐级审批</el-button>
          <el-button plain type="primary" icon="el-icon-document-delete" @click='handleFunction("指令信息字段变更审批结果拒绝和驳回")'>指令信息字段变更审批结果拒绝和驳回</el-button>
        </div>
      </div>
      <cs-pagetable
    ...
    <!-- 流程图查看弹窗 -->
    <el-dialog :title="flowchartTitle" :visible.sync="flowchartVisible" width="760px" append-to-body>
      <el-steps :active="flowchartActive" align-center style="margin-bottom:24px;">
        <el-step v-for="node in flowchartNodes" :key="node.step" :title="node.step" :description="node.status"></el-step>
      </el-steps>
      <el-table :data="flowchartNodes" border size="small" style="width:100%">
        <el-table-column prop="step" label="流程节点" width="120"></el-table-column>
        <el-table-column prop="handler" label="处理人" width="100"></el-table-column>
        <el-table-column prop="time" label="处理时间" width="160"></el-table-column>
        <el-table-column prop="status" label="状态" width="100" align="center">
          <template slot-scope="scope">
    ...
    <!-- 拒绝和驳回弹窗 -->
    <el-dialog :title="rejectTitle" :visible.sync="rejectVisible" width="660px" append-to-body>
      <el-form :model="rejectForm" label-width="120px" style="margin-bottom:16px;">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="驳回类型">
              <el-select v-model="rejectForm.type" style="width:100%">
                <el-option label="内容驳回" value="内容驳回"></el-option>
                <el-option label="格式驳回" value="格式驳回"></el-option>
                <el-option label="权限驳回" value="权限驳回"></el-option>
                <el-option label="合规驳回" value="合规驳回"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="驳回级别">
              <el-select v-model="rejectForm.level" style="width:100%">
                ...
                <el-option label="终级驳回" value="终级驳回"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="驳回原因">
          <el-input v-model="rejectForm.reason" type="textarea" :rows="2" placeholder="请输入驳回原因"></el-input>
        </el-form-item>
        <el-form-item label="备注说明">
          <el-input v-model="rejectForm.comment" type="textarea" :rows="2" placeholder="补充说明"></el-input>
        </el-form-item>
      </el-form>
      <el-table :data="rejectNodes" border size="small" style="width:100%">
        <el-table-column prop="node" label="驳回节点" width="110"></el-table-column>
        <el-table-column prop="handler" label="处理人" width="100"></el-table-column>
        <el-table-column prop="time" label="驳回时间" width="160"></el-table-column>
        <el-table-column prop="type" label="类型" width="100"></el-table-column>
        <el-table-column prop="status" label="状态" width="80" align="center">
          <template slot-scope="scope">
            <el-tag :type="scope.row.status === '已驳回' ? 'danger' : scope.row.status === '处理中' ? 'warning' : 'info'" size="mini">{{ scope.row.status }}</el-tag>
          </template>
        </el-table-column>
      </el-table>
      <span slot="footer">
    ...
    <!-- 指派信息弹窗 -->
    <el-dialog :title="assignTitle" :visible.sync="assignVisible" width="660px" append-to-body>
      <el-form :model="assignForm" label-width="120px" style="margin-bottom:16px;">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="指派单号">
              <el-input v-model="assignForm.billNo" disabled></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="指派状态">
              <el-tag :type="assignForm.status === '已指派' ? 'success' : assignForm.status === '处理中' ? 'warning' : 'info'">{{ assignForm.status }}</el-tag>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="指派人">
              <el-select v-model="assignForm.assigner" style="width:100%">
                <el-option v-for="p in personPool" :key="p" :label="p" :value="p"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-form-item label="所属部门">
          <el-select v-model="assignForm.dept" style="width:100%">
            <el-option label="信息安全部" value="信息安全部"></el-option>
            <el-option label="系统运维部" value="系统运维部"></el-option>
            <el-option label="数据管理部" value="数据管理部"></el-option>
            <el-option label="网络管理部" value="网络管理部"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="指派事项">
          <el-input v-model="assignForm.content" type="textarea" :rows="2"></el-input>
        </el-form-item>
        <el-form-item label="指派说明">
          <el-input v-model="assignForm.description" type="textarea" :rows="2"></el-input>
        </el-form-item>
        <el-form-item label="指派时间">
          <span>{{ assignForm.time }}</span>
        </el-form-item>
      </el-form>
      <span slot="footer">
...
const initialRows = [
  {
    ...
    "变更细节": "超时",
    "审批状态": "超时",
    "指令信息字段变更审批ID": "U-42348",
    "指令信息字段变更审批用户": "张三",
    "指令信息字段变更审批编号": "U-42348",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-1",
    "新增指令信息字段变更审批状态": "超时"
  },
  {
    "ID": 2,
    "指令信息字段变更审批流程ID": "U-50267",
    "指令ID": "U-50267",
    "请求者ID": "U-50267",
    "审批者ID": "U-50267",
    "变更类型": "已取消",
    "变更细节": "已取消",
    "审批状态": "已取消",
    "指令信息字段变更审批ID": "U-50267",
    "指令信息字段变更审批用户": "李四",
    "指令信息字段变更审批编号": "U-50267",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-2",
    "新增指令信息字段变更审批状态": "已取消"
  },
  {
    "ID": 3,
    "指令信息字段变更审批流程ID": "U-58186",
    "指令ID": "U-58186",
    "请求者ID": "U-58186",
    "审批者ID": "U-58186",
    "变更类型": "已驳回",
    "变更细节": "已驳回",
    "审批状态": "已驳回",
    "指令信息字段变更审批ID": "U-58186",
    "指令信息字段变更审批用户": "王五",
    "指令信息字段变更审批编号": "U-58186",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-3",
    "新增指令信息字段变更审批状态": "已驳回"
  },
  {
    "ID": 4,
    "指令信息字段变更审批流程ID": "U-66105",
    "指令ID": "U-66105",
    "请求者ID": "U-66105",
    "审批者ID": "U-66105",
    "变更类型": "待审核",
    "变更细节": "待审核",
    "审批状态": "待审核",
    "指令信息字段变更审批ID": "U-66105",
    "指令信息字段变更审批用户": "赵六",
    "指令信息字段变更审批编号": "U-66105",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-4",
    "新增指令信息字段变更审批状态": "待审核"
  },
  {
    "ID": 5,
    "指令信息字段变更审批流程ID": "U-74024",
    "指令ID": "U-74024",
    "请求者ID": "U-74024",
    "审批者ID": "U-74024",
    "变更类型": "已处理",
    "变更细节": "已处理",
    "审批状态": "已处理",
    "指令信息字段变更审批ID": "U-74024",
    "指令信息字段变更审批用户": "system",
    "指令信息字段变更审批编号": "U-74024",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-5",
    "新增指令信息字段变更审批状态": "已处理"
  },
  {
    "ID": 6,
    "指令信息字段变更审批流程ID": "U-81943",
    "指令ID": "U-81943",
    "请求者ID": "U-81943",
    "审批者ID": "U-81943",
    "变更类型": "处理中",
    "变更细节": "处理中",
    "审批状态": "处理中",
    "指令信息字段变更审批ID": "U-81943",
    "指令信息字段变更审批用户": "admin",
    "指令信息字段变更审批编号": "U-81943",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-6",
    "新增指令信息字段变更审批状态": "处理中"
  },
  {
    "ID": 7,
    "指令信息字段变更审批流程ID": "U-89862",
    "指令ID": "U-89862",
    "请求者ID": "U-89862",
    "审批者ID": "U-89862",
    "变更类型": "待处理",
    "变更细节": "待处理",
    "审批状态": "待处理",
    "指令信息字段变更审批ID": "U-89862",
    "指令信息字段变更审批用户": "张三",
    "指令信息字段变更审批编号": "U-89862",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-7",
    "新增指令信息字段变更审批状态": "待处理"
  },
  {
    "ID": 8,
    "指令信息字段变更审批流程ID": "U-97781",
    "指令ID": "U-97781",
    "请求者ID": "U-97781",
    "审批者ID": "U-97781",
    "变更类型": "处理失败",
    "变更细节": "处理失败",
    "审批状态": "处理失败",
    "指令信息字段变更审批ID": "U-97781",
    "指令信息字段变更审批用户": "李四",
    "指令信息字段变更审批编号": "U-97781",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-8",
    "新增指令信息字段变更审批状态": "处理失败"
  },
  {
    "ID": 9,
    "指令信息字段变更审批流程ID": "U-105700",
    "指令ID": "U-105700",
    "请求者ID": "U-105700",
    "审批者ID": "U-105700",
    "变更类型": "已暂停",
    "变更细节": "已暂停",
    "审批状态": "已暂停",
    "指令信息字段变更审批ID": "U-105700",
    "指令信息字段变更审批用户": "王五",
    "指令信息字段变更审批编号": "U-105700",
    "指令信息字段变更审批名称": "指令信息字段变更审批流程管理-9",
    "新增指令信息字段变更审批状态": "已暂停"
  }
];
const tableColumns = [
...
function getActionType(name) {
  ...
  if (/发起.*审批|发起.*流程/.test(name)) return "initiate_audit";
  if (/指派/.test(name)) return "assign";
...
export default {
  ...
  methods: {
    ...
    openAssignDialog(functionName, S, now) {
      ...
      this.assignForm = {
        billNo: "ASG-" + String(S).slice(-6),
        status: pick(["已指派", "待接收", "处理中", "已完成"], 0, S),
        assigner: pick(pool, 0, S),
        assignee: pick(pool, 1, S),
        dept: pick(["信息安全部", "系统运维部", "数据管理部", "网络管理部"], 2, S),
        content: functionName.replace(/^(新增|指派|信息)/g, "").trim() || "审批流程指派",
        description: "请负责人处理" + functionName + "相关事宜",
        time: now,
      };
      this.assignVisible = true;
    },
    confirmAssign() {
      this.$confirm("确认指派给【" + this.assignForm.assignee + "】？", "提示", {
        confirmButtonText: "确认", cancelButtonText: "取消", type: "info",
      }).then(() => {
        this.assignForm.status = "已指派";
        this.$message.success("指派成功，已通知 " + this.assignForm.assignee);
    ...
    openFlowchartDialog(functionName, S, now) {
      ...
      const pool = ["张伟", "李娜", "王强", "赵敏", "刘洋", "陈静"];
      this.flowchartNodes = [
        { step: "发起申请", handler: pick(pool, 0, S), time: now, status: "已完成", remark: "申请已提交，等待审批" },
        { step: "部门初审", handler: pick(pool, 1, S), time: pick([now, "-"], 0, S), status: pick(["已完成", "进行中"], 0, S), remark: "初审中，请等待" },
        { step: "技术审核", handler: pick(pool, 2, S), time: pick([now, "-"], 1, S), status: pick(["已完成", "进行中", "等待中"], 1, S), remark: "技术合规审核" },
        { step: "安全审核", handler: pick(pool, 3, S), time: pick([now, "-"], 2, S), status: pick(["已完成", "等待中", "待处理"], 2, S), remark: "安全策略检查" },
        { step: "分管审批", handler: pick(pool, 4, S), time: pick([now, "-"], 3, S), status: pick(["已完成", "待处理"], 3, S), remark: "分管领导审批" },
        { step: "归档完成", handler: "system", time: pick([now, "-"], 4, S), status: pick(["已完成", "待处理"], 4, S), remark: "流程归档" },
      ];
      this.flowchartActive = this.flowchartNodes.findIndex(n => n.status === "进行中" || n.status === "等待中" || n.status === "待处理");
      if (this.flowchartActive === -1) this.flowchartActive = this.flowchartNodes.length;
    ...
    openRejectDialog(functionName, S, now) {
      ...
      this.rejectForm = {
        type: pick(["内容驳回", "格式驳回", "权限驳回", "合规驳回"], 0, S),
        level: pick(["一级驳回", "二级驳回", "三级驳回", "终级驳回"], 1, S),
        reason: pick(["数据不完整，缺少必要字段", "格式不符合规范要求", "权限不足，无法审批", "内容与实际情况不符"], 2, S),
        comment: pick(["请补充完整后重新提交", "请按规范格式调整后重试", "请联系上级部门授权", "请核实数据后重新提交"], 3, S),
      };
      const pool = ["张伟", "李娜", "王强", "赵敏", "刘洋", "陈静"];
      this.rejectNodes = [
        { node: "初审驳回", handler: pick(pool, 0, S), time: now, type: pick(["内容驳回", "格式驳回"], 0, S), status: "已驳回" },
        { node: "复审驳回", handler: pick(pool, 1, S), time: now, type: pick(["权限驳回", "合规驳回"], 1, S), status: pick(["已驳回", "处理中"], 1, S) },
        { node: "终审驳回", handler: pick(pool, 2, S), time: pick([now, "-"], 2, S), type: pick(["内容驳回", "合规驳回"], 2, S), status: pick(["已驳回", "待处理"], 2, S) },
      ];
      this.rejectVisible = true;
    },
    confirmReject() {
      this.$confirm("确认驳回？驳回后将退回上一级处理", "提示", {

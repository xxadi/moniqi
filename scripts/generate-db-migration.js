/**
 * 生成"数据库国产化改造"模块的 Vue 页面和路由
 * 数据来源：src/data/功能点备注.json（模块 === '数据库国产化改造'）
 * 菜单位置：资产信息上报调整 > 数据库国产化改造 > {一级功能}
 * 按钮：每个二级功能一个按钮
 */
const fs = require("fs");
...
const dataPath = path.join(__dirname, "..", "src", "data", "功能点备注.json");
const allData = JSON.parse(fs.readFileSync(dataPath, "utf8"));
const dbRecords = allData.filter((r) => r.模块 === "数据库国产化改造");

console.log("数据库国产化改造记录数:", dbRecords.length);
...
dbRecords.forEach((r) => {
  const l1 = r.一级功能;
  if (!l1) return;
  if (!groups[l1]) groups[l1] = { level3: l1, functions: [] };
  if (r.二级功能) {
    groups[l1].functions.push(r.二级功能);
  }
});

const groupList = Object.values(groups);
console.log("一级功能数:", groupList.length);
...
function createMockData(group) {
  const S = pageHash(group.level3);
  const count = (S % 5) + 6;
  const times = [
  ...
  const columns = [
    ...
    { prop: "任务编号", label: "任务编号", type: "text", search: true, showTooltip: true },
    { prop: "模块名称", label: "模块名称", type: "text", search: true, showTooltip: true },
    { prop: "线程数", label: "线程数", type: "text", search: false, showTooltip: true },
    { prop: "处理状态", label: "处理状态", type: "text", search: true, showTooltip: true },
    { prop: "负载均衡策略", label: "负载均衡策略", type: "text", search: false, showTooltip: true },
    { prop: "耗时(ms)", label: "耗时(ms)", type: "text", search: false, showTooltip: true },
    { prop: "更新时间", label: "更新时间", type: "text", search: false, showTooltip: true, width: 170 },
    { slot: "operate", label: "操作" },
  ];

  const rows = Array.from({ length: count }).map((_, i) => ({
    ID: i + 1,
    任务编号: `DB-${String(S + i * 7919).slice(-6)}`,
    模块名称: pick([
      "资产管理模块", "资产清单模块", "任务调度模块", "登录采集模块",
      "远程扫描模块", "Agent代理模块", "流量分析模块", "拨测模块",
      "资产总览模块", "资产画像模块", "工信部上报模块", "脆弱性管理模块",
    ], i, S),
    线程数: pick([2, 4, 8, 16, 32, 64], i, S),
    处理状态: pick(["处理中", "已完成", "待处理", "处理失败", "已暂停", "排队中", "部分完成"], i, S),
    负载均衡策略: pick(["轮询分配", "最少连接", "加权轮询", "源地址哈希", "随机分配", "一致性哈希"], i, S),
    "耗时(ms)": pick([23, 56, 128, 256, 512, 1024, 2048], i, S),
    更新时间: t(i),
  }));
...
function createButtonsHTML(functions) {
  return functions
    .map((fn) => {
      const action = getActionType(fn);
      const icon = getButtonIcon(fn);
      return `          <el-button plain type="primary" icon="${icon}" @click='handleFunction("${fn}", "${action}")'>${fn}</el-button>`;
    })
    .join("\n");
}
...
function generateVuePage(group, componentIndex) {
  const { columns, rows } = createMockData(group);
  const buttonsHTML = createButtonsHTML(group.functions);
  ...
  importRowObj["导入来源"] = group.level3 + "导入模板.xlsx";
...
export default {
  name: "GeneratedFeaturePage${componentIndex}",
  data() {
    return {
      searchData: {},
      searchConfig: [],
  ...
  methods: {
    ...
    buildExportRows(functionName) {
      ...
      return [
        ...
        { 业务名称: "${group.level3}业务记录", 业务状态: "已处理", 处理时间: this.now(), 说明: "模拟业务记录" },
        { 业务名称: "${group.level3}复核记录", 业务状态: "待复核", 处理时间: this.now(), 说明: "等待人工确认" },
      ];
    },
    openDetail(row) {
      this.detailTitle = "详情";
...
function generateRoutes(groupList) {
  const level1Path = toPinyin("资产信息上报调整");
  const level2Name = "数据库国产化改造";
  const level2Path = toPinyin(level2Name);

  const children = groupList.map((g, i) => {
    const l3Path = toPinyin(g.level3);
    const name = `${g.level3}-4-${groupList.length}-${i + 1}`;
    return `        {
          path: "${l3Path}",
          name: "${name}",
          meta: { title: "${g.level3}" },
          component: () => import("@/views/资产信息上报调整/数据库国产化改造/${g.level3}/index"),
        }`;
  });
...
// 1. 生成 Vue 页面
const viewsDir = path.join(__dirname, "..", "src", "views", "资产信息上报调整", "数据库国产化改造");
let pageCount = 0;
groupList.forEach((group, i) => {
  const dir = path.join(viewsDir, group.level3);
  fs.mkdirSync(dir, { recursive: true });
  const vue = generateVuePage(group, 4000 + i);
...
// 找到 component: Layout（资产信息上报调整的父路由使用 Layout）
...
const routesSnippet = generateRoutes(groupList);

/**
 * 生成"YN云南COSMIC"模块的 Vue 页面和路由
 * 数据来源：功能点拆分表.json（从 Excel 提取）
 * 菜单位置：资产信息上报调整 > YN云南COSMIC > {功能用户需求}
 * 按钮：每个触发事件一个按钮
 * 表格列：根据数据属性解析生成
 */
const fs = require("fs");
...
const pages = parseExcelData();
console.log("功能用户需求（页面）数:", pages.length);
console.log("触发事件（按钮）总数:", pages.reduce((sum, p) => sum + p.triggers.length, 0));
...
function getActionType(name) {
  ...
  if (/监控告警|异常告警|失败告警/.test(name)) return "operation";
  if (/结果监控|状态监控|运行监控|采集监控/.test(name)) return "query";
  ...
  if (/采集|转换|预处理|提取|识别|分级|聚合|计算|审查|校验|研判/.test(name)) return "analysis";
...
function createColumnsFromAttrs(page) {
  ...
  attrList.forEach((attr) => {
    ...
    columns.push({ prop, label: attr, type, search, showTooltip, ...(width ? { width } : {}) });
  });
...
const DIALOG_TYPES = [
  ...
  "initiate_audit", "approval"
];

function assignDialogTypes(triggers) {
  // Each button gets its own preferred dialog type — no uniqueness constraint.
  // Multiple buttons may share the same type (e.g. multiple "新增" → form).
  return triggers.map((tr) => ({
    name: tr.name,
    dialogType: getPreferredDialogType(tr.name),
  }));
}
...
function getPreferredDialogType(name) {
  ...
  if (/分析|评估|运行效果/.test(name)) return "analysis";
  if (/删除|移除/.test(name)) return "operation";
  if (/驳回|拒绝/.test(name)) return "approval";
  // More specific keywords before the catch-all "审批" rule
  if (/流程图/.test(name)) return "preview";
  if (/查询|查看|展示|跟踪|读取/.test(name)) return "detail";
  if (/上传|附件/.test(name)) return "form";
  if (/流程图/.test(name)) return "preview";
  ...
  // Catch-all 审批/通知 must be last among keyword rules
  if (/告警|稽核|审批|通知|推送|催办|提醒/.test(name)) return "audit";
  return "operation";
}

function createButtonsHTML(triggers) {
  const assignments = assignDialogTypes(triggers);
  return assignments
    .map(({ name, dialogType }) => {
...
function generateVuePage(page, componentIndex) {
...
function generateRoutes(pages) {
  ...
  const childRoutes = pages.map((page, index) => {
    const pinyinPath = toPinyin(page.name);
    return `        {
          path: "${pinyinPath}",
          name: "${page.name}-4-${totalCount}-${index + 1}",
          meta: { title: "${page.name}" },
          component: () => import("@/views/资产信息上报调整/YN云南COSMIC/${page.name}/index"),
        }`;
  }).join(",\n");

  return `    // --- YN云南COSMIC（自动生成）---
    {
      path: "YNYunNanCOSMIC",
      name: "YN云南COSMIC",
...
const importDialogPath = path.join(__dirname, "..", "src", "views", "template", "ImportDialog.vue");
const importDialogContent = fs.readFileSync(importDialogPath, "utf8");

const existingImportDialog = path.join(__dirname, "..", "src", "views", "资产信息上报调整", "JL吉林COSMIC", "IP扫描指令下发接口", "ImportDialog.vue");
const importDialogTemplate = fs.existsSync(existingImportDialog)
  ? fs.readFileSync(existingImportDialog, "utf8")
  : importDialogContent;
...
for (let i = 0; i < pages.length; i++) {
  const page = pages[i];
  const pageDir = path.join(viewsDir, page.name);

  if (!fs.existsSync(pageDir)) {
    fs.mkdirSync(pageDir, { recursive: true });
  }

  const vueContent = generateVuePage(page, 6000 + i);
  ...
  const importPath = path.join(pageDir, "ImportDialog.vue");
  if (!fs.existsSync(importPath)) {
    fs.writeFileSync(importPath, importDialogTemplate, "utf8");
  }
}

console.log("创建页面数:", createdCount);
console.log("跳过已有页面数:", skippedCount);

const routeBlock = generateRoutes(pages);
...
const ynMarker = "// --- YN云南COSMIC（自动生成）---";
// Remove ALL existing YN blocks (handles duplicates from multiple runs)
while (examplesContent.includes(ynMarker)) {
  const ynStartIdx = examplesContent.indexOf(ynMarker);
  let bracketDepth = 0;
  let blockStart = ynStartIdx;
  while (blockStart > 0 && examplesContent[blockStart] !== "{") blockStart--;
  blockStart--;
  let blockEnd = blockStart;
  for (let i = blockStart; i < examplesContent.length; i++) {
    if (examplesContent[i] === "{") bracketDepth++;
    if (examplesContent[i] === "}") bracketDepth--;
    if (bracketDepth === 0) {
      blockEnd = i + 1;
      break;
  ...
  examplesContent = examplesContent.slice(0, blockStart) + examplesContent.slice(blockEnd);
  console.log("已移除旧的 YN 路由块");
}

const parentMarker = 'name: "资产信息上报调整"';
const parentIdx = examplesContent.indexOf(parentMarker);
if (parentIdx === -1) {
  console.error("未找到资产信息上报调整路由");
  process.exit(1);
}

let searchStart = parentIdx;
let childrenIdx = examplesContent.indexOf("children: [", searchStart);
if (childrenIdx === -1) {
  console.error("未找到 children 数组");
  process.exit(1);
}

let depth = 0;
let childrenEnd = -1;
for (let i = childrenIdx + "children: [".length; i < examplesContent.length; i++) {
  if (examplesContent[i] === "[") depth++;
  if (examplesContent[i] === "]") {
    if (depth === 0) {
      childrenEnd = i;
      break;
    }
...
if (childrenEnd === -1) {
  console.error("未找到 children 数组结束位置");
  process.exit(1);
}

examplesContent = examplesContent.slice(0, childrenEnd) + "\n" + routeBlock + "\n      " + examplesContent.slice(childrenEnd);

fs.writeFileSync(examplesPath, examplesContent, "utf8");
console.log("路由已更新");
console.log("YN云南COSMIC 模块生成完成！");

...
files.forEach(filePath => {
  ...
  // 2. Add dialog template - insert before the last </el-dialog> that's followed by \n  </div>\n</template>
  if (!content.includes("assignVisible")) {
    // Find the last </el-dialog> before </div></template>
    const templateEnd = content.indexOf("</template>");
    if (templateEnd !== -1) {
      const beforeTemplate = content.substring(0, templateEnd);
      const lastDialogClose = beforeTemplate.lastIndexOf("</el-dialog>");
      if (lastDialogClose !== -1) {
        const insertPos = lastDialogClose + "</el-dialog>".length;
        content = content.substring(0, insertPos) + "\n" + DIALOG_TEMPLATE + content.substring(insertPos);
        changed = true;
      }
    }
  }
  ...
  if (!content.includes("assignVisible:")) {
    // Try to insert after uploadHistory or after the last data property before };
    const dataEndMatch = content.match(/uploadHistory: \[\],\n/);
    if (dataEndMatch) {
      content = content.replace("uploadHistory: [],\n", "uploadHistory: [],\n" + DATA_PROPS + "\n");
      changed = true;
    } else {
      // Try inserting before the closing of data()
      const dataCloseMatch = content.match(/(      \w+Visible: false,\n      \w+Title: "[^"]*",\n)/g);
      if (dataCloseMatch && dataCloseMatch.length > 0) {
        const lastMatch = dataCloseMatch[dataCloseMatch.length - 1];
        const lastIdx = content.lastIndexOf(lastMatch);
        const insertAfter = lastIdx + lastMatch.length;
        content = content.substring(0, insertAfter) + DATA_PROPS + "\n" + content.substring(insertAfter);
        changed = true;
      } else {
  ...
  if (!content.includes('case "assign":')) {
    // Try to insert after notify case
  ...
  if (!content.includes("openAssignDialog")) {
    // Try to insert after sendNotify
    if (content.includes("sendNotify()")) {
      content = content.replace(
        /(sendNotify\(\) \{[\s\S]*?this\.\$message\.success\("通知发送成功"\);\n    \},)/,
        "$1\n" + METHODS
      );
      changed = true;
    } else {
